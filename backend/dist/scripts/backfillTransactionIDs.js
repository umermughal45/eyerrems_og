...
async function backfillTransactionIDs() {
    try {
        logger_1.default.info('Starting T-ID Backfill Process...');
        // 1. Process Leads
        ...
        for (const lead of leads) {
            ...
            if (!tid) {
                ...
                logger_1.default.info(`Generated new T-ID for Lead ${lead.id}: ${tid}`);
            }
            if (tid) {
                await registerTID(tid, 'lead', lead.id, 'CRM');
            }
        }
        logger_1.default.info(`Processed ${leads.length} Leads`);
        ...
        logger_1.default.info(`Processed ${clients.length} Clients`);
        ...
        logger_1.default.info(`Processed ${deals.length} Deals`);
        ...
        logger_1.default.info(`Processed ${invoices.length} Invoices`);
        ...
        logger_1.default.info(`Processed ${payments.length} Payments`);
        logger_1.default.info('T-ID Backfill Process Complete');
    }
    catch (error) {
        logger_1.default.error(`Error during T-ID backfill: ${error}`);
    }
    finally {
...
async function registerTID(tid, entityType, entityId, moduleName) {
    try {
        ...
        if (!existing) {
            await transactionIdentity_service_1.TransactionIdentityEngine.attachTid(tid, entityType, entityId, moduleName);
        }
    }
    catch (error) {
        logger_1.default.warn(`Failed to register TID ${tid} for ${entityType} ${entityId}: ${error.message}`);
    }
}
...
//# sourceMappingURL=backfillTransactionIDs.js.map