 ...
 */
async function hasPermission(roleId, roleName, userPermissions, requiredPermission) {
    try {
        ...
        if (roleStatus === 'DEACTIVATED') {
            logger_1.default.info(`Permission denied: Role ${roleName} (${roleId}) is DEACTIVATED`);
        ...
        if (isAdmin && !explicitCheck.allowed) {
            ...
            logger_1.default.warn(`Admin role missing explicit permission: ${requiredPermission}. Consider adding via migration.`);
        }
        ...
        if (hasLegacyWildcard) {
            logger_1.default.info(`Legacy wildcard detected for ${roleName}, auto-converting permissions`);
            // Auto-convert legacy permissions on first access
            try {
                await (0, compatibility_resolver_1.resolveRolePermissions)(roleId, userPermissions);
                ...
                logger_1.default.info(`After conversion, permission ${requiredPermission} = ${retryCheck.allowed}`);
                ...
                if (isAdmin) {
                    ...
                    if (allPermsList.includes(requiredPermission)) {
                        logger_1.default.info(`✅ Granting ${requiredPermission} to Admin after conversion (available permission)`);
                        return true;
                    }
                }
            }
                ...
                logger_1.default.warn(`Permission conversion failed for role ${roleName}, using legacy check: ${error.message}`);
        ...
        logger_1.default.error(`Permission check error for role ${roleName}: ${error.message}`, error);
...
function hasPermissionSync(userPermissions, requiredPermission) {
...
function requirePermission(permission) {
    return async (req, res, next) => {
        ...
        const hasRequiredPermission = await hasPermission(req.user.roleId, req.user.role.name, req.user.role.permissions, permission);
        ...
        await (0, audit_logger_1.logActionExecution)({
            userId: req.user.id,
            username: req.user.username,
            roleId: req.user.roleId,
...
function requireAnyPermission(permissions) {
    return async (req, res, next) => {
        ...
        for (const permission of permissions) {
            const result = await hasPermission(req.user.roleId, req.user.role.name, req.user.role.permissions, permission);
            if (result) {
