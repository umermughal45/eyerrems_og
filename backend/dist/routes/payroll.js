...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const payroll = await client_1.default.payroll.findMany({
            where,
            include: {
                employee: {
                    select: {
        ...
        const formattedPayroll = await Promise.all(payroll.map(async (record) => {
            // Calculate paid amount from payments
            const payments = await client_1.default.payrollPayment.findMany({
            ...
            if (record.paidAmount !== paidAmount || record.paymentStatus !== status) {
                await client_1.default.payroll.update({
                    where: { id: record.id },
                    data: {
                        paidAmount,
                        remainingBalance,
...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        const { employeeId, month, baseSalary, basicSalary, bonus, overtimeAmount, overtimeHours, taxPercent, taxAmount, grossSalary, allowances, deductions, netPay, paymentMethod, paymentStatus, notes, allowancesList, deductionsList } = req.body;
        ...
        const payroll = await client_1.default.payroll.create({
            data: {
                employeeId,
                month,
                baseSalary: base,
        ...
        if (Array.isArray(allowancesList) && allowancesList.length > 0) {
            await Promise.all(allowancesList.map((allowance) => client_1.default.payrollAllowance.create({
                data: {
        ...
        if (Array.isArray(deductionsList) && deductionsList.length > 0) {
            await Promise.all(deductionsList.map((deduction) => client_1.default.payrollDeduction.create({
                data: {
                    payrollId: payroll.id,
                    employeeId,
        ...
        try {
            const { PayrollAccountingService } = await Promise.resolve().then(() => __importStar(require('../services/payroll-accounting-service')));
        ...
        const payrollWithDetails = await client_1.default.payroll.findUnique({
            where: { id: payroll.id },
            include: {
                employee: {
                    select: {
...
router.put('/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const { baseSalary, basicSalary, bonus, overtimeAmount, allowances, deductions, taxPercent, taxAmount, grossSalary, netPay, paymentStatus, paymentDate, paymentMethod, status } = req.body;
        const oldPayroll = await client_1.default.payroll.findUnique({
            where: { id },
        });
        ...
        if (oldPayroll.journalEntryId) {
            ...
            const isModifyingFinancials = baseSalary !== undefined ||
                bonus !== undefined ||
        ...
        const updatedPayroll = await client_1.default.payroll.update({
            where: { id },
            data: {
...
router.get('/reminders', auth_1.authenticate, async (req, res) => {
    try {
        const { getPayrollReminders } = await Promise.resolve().then(() => __importStar(require('../services/hr-alerts')));
...
router.post('/calculate-overtime', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { calculateMonthlyOvertime } = await Promise.resolve().then(() => __importStar(require('../services/hr-alerts')));
...
router.post('/calculate-attendance-based', auth_1.authenticate, async (req, res) => {
    try {
        const { employeeId, month, baseSalary, absentDeductionRate, lateDeductionRate, halfDayDeductionRate, overtimeHourlyRate, standardWorkingHours, standardWorkingDays, includeLeaveDays, } = req.body;
        if (!employeeId || !month || !baseSalary) {
            return res.status(400).json({
                success: false,
        ...
        const { AttendancePayrollIntegrationService } = await Promise.resolve().then(() => __importStar(require('../services/attendance-payroll-integration-service')));
...
router.get('/attendance-summary/:employeeId/:month', auth_1.authenticate, async (req, res) => {
    try {
...
router.post('/:id/payments', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const { PayrollPaymentSafetyService } = await Promise.resolve().then(() => __importStar(require('../services/payroll-payment-safety-service')));
        ...
        const { PayrollAccountingService } = await Promise.resolve().then(() => __importStar(require('../services/payroll-accounting-service')));
        const coaValidation = await PayrollAccountingService.validatePaymentPosting(id, paymentAmount);
...
router.get('/analytics/payments', auth_1.authenticate, async (req, res) => {
    try {
        const { startDate, endDate, department, employeeId } = req.query;
        const { PayrollPaymentAnalyticsService } = await Promise.resolve().then(() => __importStar(require('../services/payroll-payment-analytics-service')));
...
router.get('/reconciliation/payments', auth_1.authenticate, async (req, res) => {
    try {
