...
const dateSegment = (date = new Date()) => {
...
const DEAL_PAYMENT_TYPES = ['token', 'booking', 'installment', 'partial', 'full'];
const DEAL_PAYMENT_MODES = ['cash', 'bank', 'online_transfer', 'card'];
const createDealPaymentSchema = zod_1.z.object({
    dealId: zod_1.z.string().uuid(),
    amount: zod_1.z.number().positive(),
    paymentType: zod_1.z.enum(DEAL_PAYMENT_TYPES),
    paymentMode: zod_1.z.enum(DEAL_PAYMENT_MODES),
...
router.get('/accounts', auth_1.authenticate, async (_req, res) => {
    try {
        ...
        const accountsWithBalances = await Promise.all(accounts.map(async (account) => {
            ...
            const debitAggregate = await client_1.default.ledgerEntry.aggregate({
                where: {
            ...
            const creditAggregate = await client_1.default.ledgerEntry.aggregate({
                where: {
...
router.get('/accounts/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.account.findUnique({ where: { id: req.params.id } });
        if (!item)
...
router.post('/accounts', auth_1.authenticate, async (req, res) => {
    try {
        const { code, name, type, description, parentId, isPostable, cashFlowCategory } = req.body;
        if (!code || !name || !type) {
            return res.status(400).json({ error: 'code, name and type are required' });
        }
        // Validate parentId if provided
        if (parentId) {
            const parentAccount = await client_1.default.account.findUnique({
                where: { id: parentId }
            });
            if (!parentAccount) {
        ...
        const item = await client_1.default.account.create({
            data: accountData,
        });
...
router.put('/accounts/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.account.update({
...
router.delete('/accounts/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.account.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch (error) {
...
router.get('/transaction-categories', auth_1.authenticate, async (_req, res) => {
    try {
        const items = await client_1.default.transactionCategory.findMany({
            orderBy: { name: 'asc' },
            include: {
        ...
        logger_1.default.error('Get transaction categories error:', error);
...
router.post('/transaction-categories', auth_1.authenticate, async (req, res) => {
    try {
        const { name, type, description, defaultDebitAccountId, defaultCreditAccountId } = req.body;
        if (!name || !type) {
            return res.status(400).json({ error: 'name and type are required' });
        }
        const item = await client_1.default.transactionCategory.create({
            data: {
                name,
                type,
                description: description || null,
        ...
        logger_1.default.error('Create transaction category error:', error);
...
router.put('/transaction-categories/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.transactionCategory.update({
            where: { id: req.params.id },
            data: {
                name: req.body.name,
                type: req.body.type,
...
router.delete('/transaction-categories/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.transactionCategory.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch (error) {
        logger_1.default.error('Delete transaction category error:', error);
...
router.get('/transactions', auth_1.authenticate, async (req, res) => {
    try {
        ...
        return (0, error_handler_1.successResponse)(res, items, 200, pagination);
    }
    catch (error) {
        logger_1.default.error('Fetch transactions failed:', error);
        res.status(500).json({ error: 'Failed to fetch transactions' });
    }
});
router.get('/transactions/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.transaction.findUnique({
            where: { id: req.params.id },
            include: {
                transactionCategory: true,
                debitAccount: true,
...
router.post('/transactions', auth_1.authenticate, async (req, res) => {
    try {
        const { transactionType, amount, taxAmount, debitAccountId, creditAccountId, transactionCategoryId, paymentMethod, description, date, tenantId, assignToTenantId, dealerId, assignToDealerId, propertyId, assignToPropertyId, invoiceId, status, attachments, } = req.body;
        ...
        if (!resolvedDebitAccountId && !resolvedCreditAccountId && transactionCategoryId) {
            const category = await client_1.default.transactionCategory.findUnique({
...
router.put('/transactions/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const item = await client_1.default.transaction.update({
            where: { id: req.params.id },
            data: req.body,
        });
        res.json(item);
...
router.delete('/transactions/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.transaction.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch {
...
router.get('/invoices/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.invoice.findUnique({
            where: { id: req.params.id },
            include: {
                tenant: true,
                property: true,
...
router.post('/invoices', auth_1.authenticate, async (req, res) => {
    try {
        const { tenantId, propertyId, amount, billingDate, dueDate, taxPercent, discountAmount, lateFeeRule, lateFeeAmount, lateFeePercent, termsAndConditions, attachments, tenantAccountId, incomeAccountId, description, dealId, } = req.body;
        ...
        let tid = null;
        if (dealId) {
            const deal = await client_1.default.deal.findUnique({ where: { id: dealId } });
            tid = deal?.tid || null;
        }
        else if (propertyId) {
            const property = await client_1.default.property.findUnique({ where: { id: propertyId } });
        ...
        if (tid) {
            await transactionIdentity_service_1.TransactionIdentityEngine.attachTid(tid, 'invoice', created.id, 'Finance');
        }
        // Auto-sync to Finance Ledger
        await (0, workflows_1.syncInvoiceToFinanceLedger)(created.id);
        // Update tenant ledger if tenant exists
        if (created.tenantId) {
            await (0, workflows_1.updateTenantLedger)(created.tenantId, {
                entryType: 'debit',
...
router.put('/invoices/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const item = await client_1.default.invoice.update({
            where: { id: req.params.id },
            data: req.body,
        });
        res.json(item);
...
router.delete('/invoices/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.invoice.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch {
...
// Payments
router.get('/payments', auth_1.authenticate, async (req, res) => {
    try {
        const { page, limit } = (0, pagination_1.parsePaginationQuery)(req.query);
        const skip = (page - 1) * limit;
        ...
        const pagination = (0, pagination_1.calculatePagination)(page, limit, total);
        return (0, error_handler_1.successResponse)(res, payments, 200, pagination);
    }
    catch (error) {
...
router.get('/payments/:id/receipt', auth_1.authenticate, async (req, res) => {
    try {
        const { generateReceiptPDF } = await Promise.resolve().then(() => __importStar(require('../utils/pdf-generator')));
        const payment = await client_1.default.payment.findUnique({
            where: { id: req.params.id },
            include: {
                deal: {
        ...
        logger_1.default.error('Generate payment receipt PDF error:', error);
        res.status(500).json({
            success: false,
...
router.post('/upload-attachment', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { relativePath, filename: secureFilename } = await saveFileSecurely(buffer, filename || `finance-attachment-${Date.now()}.${mimeType.split('/')[1]}`, 'finance', req.user.id);
        ...
        logger_1.default.error('Upload finance attachment error:', error);
        res.status(500).json({
            success: false,
...
router.get('/attachments/:id', auth_1.authenticate, async (req, res) => {
    try {
        const attachment = await client_1.default.attachment.findUnique({
            where: { id: req.params.id },
        });
        ...
        logger_1.default.error('Get finance attachment error:', error);
        res.status(500).json({
            success: false,
...
router.get('/attachments', auth_1.authenticate, async (req, res) => {
    try {
        const { transactionId, entryId } = req.query;
        const entityId = (transactionId || entryId);
        if (!entityId) {
            return res.status(400).json({
                success: false,
        ...
        logger_1.default.error('Get finance attachments error:', error);
        res.status(500).json({
            success: false,
...
router.get('/vouchers', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        logger_1.default.error('List vouchers error:', error);
...
router.post('/vouchers', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        const { type, // BPV, BRV, CPV, CRV, JV
        date, paymentMethod, // Cheque, Transfer, Online, Cash
        accountId, // Primary account (bank/cash)
        description, referenceNumber, propertyId, unitId, payeeType, // Vendor, Owner, Agent, Contractor, Tenant, Client, Dealer, Employee
        payeeId, dealId, lines, // Array of { accountId, debit, credit, description, propertyId?, unitId? }
        attachments, invoiceAllocations, // For BRV: Array of { invoiceId, amount }
        preparedByUserId, } = req.body;
        ...
        const isJV = type.toUpperCase() === 'JV';
        ...
        logger_1.default.error('Create voucher error:', error);
        const message = error?.message || 'Failed to create voucher';
        res.status(400).json({ error: message });
    }
});
// GET /finance/vouchers/:id - Get voucher by ID
router.get('/vouchers/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        logger_1.default.error('Get voucher error:', error);
        res.status(500).json({ error: error.message || 'Failed to fetch voucher' });
    }
});
// PUT /finance/vouchers/:id - Update voucher (only draft status)
router.put('/vouchers/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        const { date, paymentMethod, accountId, description, referenceNumber, propertyId, unitId, payeeType, payeeId, dealId, lines, attachments, invoiceAllocations, // For BRV: Array of { invoiceId, amount }
         } = req.body;
        const normalizedAttachments = normalizeAttachments(attachments);
        // Filter and transform attachments to match service interface
        const validAttachments = normalizedAttachments
        ...
        const updatePayload = {};
        if (date !== undefined)
            updatePayload.date = normalizeDateInput(date);
        if (paymentMethod !== undefined)
            updatePayload.paymentMethod = paymentMethod;
        if (accountId !== undefined)
            updatePayload.accountId = accountId;
        if (description !== undefined)
            updatePayload.description = description;
        if (referenceNumber !== undefined)
            updatePayload.referenceNumber = referenceNumber;
        if (propertyId !== undefined)
            updatePayload.propertyId = propertyId;
        if (unitId !== undefined)
            updatePayload.unitId = unitId;
        if (payeeType !== undefined)
            updatePayload.payeeType = payeeType;
        if (payeeId !== undefined)
            updatePayload.payeeId = payeeId;
        if (dealId !== undefined)
            updatePayload.dealId = dealId;
        if (lines !== undefined && Array.isArray(lines)) {
            updatePayload.lines = lines.map((line) => ({
                accountId: line.accountId,
                debit: parseFloat(line.debit) || 0,
                credit: parseFloat(line.credit) || 0,
                description: line.description,
        ...
        if (validAttachments.length > 0 || invoiceAllocations) {
            ...
            const existingVoucher = await client_1.default.voucher.findUnique({
                where: { id: req.params.id },
                select: { attachments: true },
            });
            let attachmentsData = null;
            if (validAttachments.length > 0 || invoiceAllocations) {
                const existingAttachments = existingVoucher?.attachments;
                attachmentsData = {
        ...
        logger_1.default.error('Update voucher error:', error);
...
router.put('/vouchers/:id/submit', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        logger_1.default.error('Submit voucher error:', error);
...
router.put('/vouchers/:id/approve', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        logger_1.default.error('Approve voucher error:', error);
...
router.put('/vouchers/:id/post', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        const voucher = await VoucherService.postVoucher(req.params.id, req.user.id, normalizedPostingDate);
        res.json({ success: true, data: voucher });
    }
    catch (error) {
        const errMsg = error?.message || 'Unknown error';
        logger_1.default.error(`Post voucher error: ${errMsg}`, {
...
router.put('/vouchers/:id/reverse', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        const result = await VoucherService.reverseVoucher(req.params.id, req.user.id, normalizeDateInput(reversalDate));
        res.json({ success: true, data: result });
    }
    catch (error) {
        logger_1.default.error('Reverse voucher error:', error);
...
router.get('/vouchers/:id/pdf', auth_1.authenticate, async (req, res) => {
    try {
        const { VoucherService } = await Promise.resolve().then(() => __importStar(require('../services/voucher-service')));
        ...
        const { generateVoucherReportPDF } = await Promise.resolve().then(() => __importStar(require('../utils/audit-grade-pdf-report')));
        const pdfData = {
            voucher: {
                ...
                checkedBy: voucher.checkedBy ? {
                    username: voucher.checkedBy.username || null,
                    email: voucher.checkedBy.email || null,
                } : null,
        ...
        logger_1.default.error('Generate voucher PDF error:', error);
        res.status(500).json({
...
router.get('/vouchers/export', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Export vouchers error:', error);
        res.status(500).json({
...
router.delete('/vouchers/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Delete voucher error:', error);
        res.status(400).json({ error: error.message || 'Failed to delete voucher' });
    }
});
// Commissions
router.get('/commissions', auth_1.authenticate, async (_req, res) => {
    try {
        const items = await client_1.default.commission.findMany({ orderBy: { createdAt: 'desc' } });
        res.json(items);
    }
    catch {
...
router.get('/commissions/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.commission.findUnique({ where: { id: req.params.id } });
        if (!item)
...
router.post('/commissions', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.commission.create({ data: req.body });
        res.status(201).json(item);
    }
    catch {
...
router.put('/commissions/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.commission.update({
            where: { id: req.params.id },
            data: req.body,
        });
        res.json(item);
...
router.delete('/commissions/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.commission.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch {
...
// -------------------- Journal Vouchers --------------------
router.get('/journals', auth_1.authenticate, async (_req, res) => {
    try {
        const items = await client_1.default.journalEntry.findMany({
            orderBy: { date: 'desc' },
            include: { lines: true },
        });
        res.json(items);
...
router.get('/journals/:id', auth_1.authenticate, async (req, res) => {
    try {
        const item = await client_1.default.journalEntry.findUnique({
            where: { id: req.params.id },
            include: { lines: true },
        });
        if (!item)
...
router.post('/journals', auth_1.authenticate, async (req, res) => {
    try {
        ...
        await account_validation_service_1.AccountValidationService.validateJournalEntry(normalizedLines);
...
router.delete('/journals/:id', auth_1.authenticate, async (req, res) => {
    try {
        await client_1.default.journalLine.deleteMany({ where: { entryId: req.params.id } });
        await client_1.default.journalEntry.delete({ where: { id: req.params.id } });
        res.status(204).end();
    }
    catch {
...
router.get('/ledgers/clients', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Client ledger error:', error);
        res.status(500).json({ error: 'Failed to fetch client ledger' });
    }
});
// GET /ledger/client/:clientId - Specific client ledger route
router.get('/ledger/client/:clientId', auth_1.authenticate, async (req, res) => {
    try {
        const { UnifiedLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/unified-ledger-service')));
...
router.get('/ledgers/properties', auth_1.authenticate, async (req, res) => {
    try {
        const { LedgerService } = await Promise.resolve().then(() => __importStar(require('../services/ledger-service')));
        const propertyId = req.query.propertyId;
        ...
        logger_1.default.error('Property ledger error:', error);
        res.status(500).json({ error: 'Failed to fetch property ledger' });
    }
});
router.get('/ledgers/company', auth_1.authenticate, async (req, res) => {
    try {
        const { LedgerService } = await Promise.resolve().then(() => __importStar(require('../services/ledger-service')));
        const filters = {};
        if (req.query.startDate)
        ...
        logger_1.default.error('Company ledger error:', error);
...
router.post('/invoices/generate-monthly', auth_1.authenticate, async (req, res) => {
    try {
        const invoices = await (0, workflows_1.generateMonthlyInvoices)();
        res.json({
            success: true,
        ...
        logger_1.default.error('Generate monthly invoices error:', error);
        res.status(500).json({
            success: false,
...
// Get rent due list
router.get('/rent-due', auth_1.authenticate, async (req, res) => {
    try {
        const { propertyId, tenantId, status } = req.query;
        const today = new Date();
        const where = {
            isDeleted: false,
            status: status ? status : { in: ['unpaid', 'partial', 'overdue'] },
        };
        if (propertyId) {
        ...
        const rentDueList = invoices.map((invoice) => {
            const daysDiff = Math.floor((invoice.dueDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        ...
        logger_1.default.error('Get rent due list error:', error);
        res.status(500).json({
            success: false,
...
router.get('/commissions', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const commissionsWithStatus = await Promise.all(commissions.map(async (commission) => {
            // Check if commission has been paid (linked to finance ledger)
            const financeLedger = await client_1.default.financeLedger.findFirst({
                where: {
        ...
        logger_1.default.error('Get commissions error:', error);
        res.status(500).json({
            success: false,
...
// Pay commission
router.post('/commissions/:id/pay', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const { paymentMethod, bankAccountId, notes, paymentDate } = req.body;
        const commission = await client_1.default.commission.findUnique({
            where: { id },
            include: {
                dealer: true,
                sale: {
        ...
        const existingLedger = await client_1.default.financeLedger.findFirst({
            where: {
                referenceType: 'commission',
                referenceId: id,
                category: 'debit',
        ...
        const ledger = await (0, workflows_1.syncCommissionToFinanceLedger)(id);
        if (!ledger) {
            return res.status(500).json({
                success: false,
        ...
        logger_1.default.error('Pay commission error:', error);
        res.status(500).json({
            success: false,
...
// Bank reconciliation
router.get('/bank-reconciliation', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const openingTransactions = await client_1.default.transaction.findMany({
            where: {
                OR: [
                    { debitAccountId: accountId },
                    { creditAccountId: accountId },
        ...
        const openingJournalEntries = await client_1.default.journalEntry.findMany({
            where: {
                lines: {
                    some: { accountId: accountId },
                },
        ...
        logger_1.default.error('Bank reconciliation error:', error);
        res.status(500).json({
            success: false,
...
router.post('/payment-plans/create', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const installmentsTotal = installments.reduce((sum, inst) => sum + (inst.amount || 0), 0);
        ...
        const paymentPlan = await client_1.default.$transaction(async (tx) => {
            ...
            const regularInstallments = await Promise.all(installments.map((inst, index) => {
...
router.post('/payment-plans', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/payment-plans', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const [plans, total] = await Promise.all([
            client_1.default.paymentPlan.findMany({
                where,
                include: {
                    deal: {
                        select: {
            ...
            client_1.default.paymentPlan.count({ where }),
        ]);
        ...
        const plansWithSummary = plans.map((plan) => {
            ...
            const overdueCount = installments.filter((i) => i.status === 'overdue' || (i.status === 'unpaid' && new Date(i.dueDate) < new Date())).length;
            return {
                ...plan,
                summary: {
        ...
        const pagination = (0, pagination_1.calculatePagination)(page, limit, total);
        return (0, error_handler_1.successResponse)(res, plansWithSummary, 200, pagination);
    }
    catch (error) {
        logger_1.default.error('Get payment plans error:', error);
        res.status(500).json({
            success: false,
...
router.get('/payment-plans/deal/:dealId', auth_1.authenticate, async (req, res) => {
    try {
        const { PaymentPlanService } = await Promise.resolve().then(() => __importStar(require('../services/payment-plan-service')));
...
router.get('/payment-plans/reports', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const summary = {
            ...
            overdue: installments.filter((i) => i.status === 'overdue' || (i.status === 'unpaid' && new Date(i.dueDate) < new Date())).length,
        ...
        logger_1.default.error('Get installment reports error:', error);
        res.status(500).json({
            success: false,
...
router.post('/installments/:id/payment', auth_1.authenticate, async (req, res) => {
    try {
        const { PaymentPlanService } = await Promise.resolve().then(() => __importStar(require('../services/payment-plan-service')));
        const installment = await PaymentPlanService.recordInstallmentPayment(req.params.id, req.body.amount, req.body.paymentMode, new Date(req.body.paymentDate || new Date()), req.body.paymentId);
        res.json({ success: true, data: installment });
    }
    catch (error) {
        logger_1.default.error('Record installment payment error:', error);
        res.status(400).json({
            success: false,
...
router.get('/receipts/:dealId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Get details receipts error:', error);
        res.status(500).json({
            success: false,
...
// Get receipt by ID
router.get('/receipts/id/:id', auth_1.authenticate, async (req, res) => {
    try {
        const receipt = await client_1.default.dealReceipt.findUnique({
            where: { id: req.params.id },
            include: {
                allocations: {
                    include: {
        ...
        logger_1.default.error('Get receipt details error:', error);
        res.status(500).json({
            success: false,
...
router.get('/receipts/pdf/:id', auth_1.authenticate, async (req, res) => {
    try {
        const receipt = await client_1.default.dealReceipt.findUnique({
            where: { id: req.params.id },
            include: {
        ...
        logger_1.default.error('Generate receipt PDF error:', error);
        if (!res.headersSent) {
            res.status(500).json({
                success: false,
...
// Delete receipt
router.delete('/receipts/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Delete receipt error:', error);
        res.status(500).json({
            success: false,
...
router.get('/receipts/export', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Export receipts error:', error);
        res.status(500).json({
            success: false,
...
// -------------------- Dealer Ledger --------------------
// Get dealer ledger
router.get('/dealer-ledger/:dealerId', auth_1.authenticate, async (req, res) => {
    try {
        const { DealerLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/dealer-ledger-service')));
        const filters = {};
        if (req.query.startDate)
            filters.startDate = new Date(req.query.startDate);
        if (req.query.endDate)
            filters.endDate = new Date(req.query.endDate);
        if (req.query.dealId)
            filters.dealId = req.query.dealId;
        if (req.query.period)
        ...
        logger_1.default.error('Get dealer ledger error:', error);
        res.status(500).json({
            success: false,
...
router.get('/ledger/dealer/:dealerId', auth_1.authenticate, async (req, res) => {
    try {
        const { DealerLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/dealer-ledger-service')));
        const filters = {};
        if (req.query.startDate)
...
router.get('/ledger/:type/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { UnifiedLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/unified-ledger-service')));
        const { type, id } = req.params;
        logger_1.default.info(`[Unified Ledger] Request received: type=${type}, id=${id}`);
        ...
        logger_1.default.info(`[Unified Ledger] Fetching ledger for ${type} with ID: ${id}`);
        const ledger = await UnifiedLedgerService.getLedger(type, id, filters);
        logger_1.default.info(`[Unified Ledger] Successfully fetched ledger for ${type} ${id}`);
        res.json({
            success: true,
            data: ledger,
        });
        ...
        logger_1.default.error(`[Unified Ledger] Error for ${req.params.type} ${req.params.id}:`, error);
...
router.post('/dealer-ledger/:dealerId/payment', auth_1.authenticate, async (req, res) => {
    try {
        const { DealerLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/dealer-ledger-service')));
        const entry = await DealerLedgerService.recordPayment(req.params.dealerId, req.body.amount, req.body.paymentMode, req.body.description, req.body.referenceId);
        res.json({ success: true, data: entry });
    }
    catch (error) {
        logger_1.default.error('Record dealer payment error:', error);
        res.status(400).json({
            success: false,
...
// Get dealer balance
router.get('/dealer-ledger/:dealerId/balance', auth_1.authenticate, async (req, res) => {
    try {
        const { DealerLedgerService } = await Promise.resolve().then(() => __importStar(require('../services/dealer-ledger-service')));
        ...
        logger_1.default.error('Get dealer balance error:', error);
        res.status(500).json({
            success: false,
...
router.put('/payment-plans/update/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
            ...
            for (const inst of installments) {
                if (inst.id) {
                    try {
                        ...
                        logger_1.default.error(`Failed to update installment ${inst.id}:`, instError);
                        return res.status(400).json({
                            success: false,
        ...
        if (notes !== undefined) {
            await client_1.default.paymentPlan.update({
                where: { id },
                data: { notes: notes || null },
            });
        }
        // Fetch updated payment plan
        const updatedPlan = await client_1.default.paymentPlan.findUnique({
            where: { id },
            include: {
                installments: { where: { isDeleted: false }, orderBy: { installmentNumber: 'asc' } },
                deal: true,
...
router.post('/receipts/create', auth_1.authenticate, async (req, res) => {
    try {
        const { ReceiptService } = await Promise.resolve().then(() => __importStar(require('../services/receipt-service')));
        ...
        logger_1.default.error('Create receipt error:', error);
        res.status(500).json({
            success: false,
...
router.get('/receipts/:dealId', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Get receipts error:', error);
        res.status(500).json({
            success: false,
...
router.get('/receipts', auth_1.authenticate, async (req, res) => {
    try {
...
router.get('/receipts/id/:id', auth_1.authenticate, async (req, res) => {
    try {
        ...
        logger_1.default.error('Get receipt error:', error);
        res.status(500).json({
            success: false,
...
router.put('/receipts/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { ReceiptService } = await Promise.resolve().then(() => __importStar(require('../services/receipt-service')));
        const { amount, method, date, notes, referenceNumber } = req.body;
        const existingReceipt = await client_1.default.dealReceipt.findUnique({
            where: { id: req.params.id },
            include: {
                allocations: true,
                journalEntry: true,
        ...
        // Update receipt
        const updatedReceipt = await client_1.default.$transaction(async (tx) => {
        ...
        logger_1.default.error('Update receipt error:', error);
        res.status(500).json({
            success: false,
