...
function getModelColumns(modelName) {
...
router.get('/export', auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    try {
        const sections = [];
        // PROPERTIES section
        ...
        const financeLedgers = await client_1.default.financeLedger.findMany({
            where: { isDeleted: false },
            orderBy: { createdAt: 'asc' },
        });
...
function parseCsvWithSections(csvContent) {
...
async function upsertProperty(tx, row, result, index) {
...
async function upsertFinanceLedger(tx, row, result, index) {
...
async function upsertClient(tx, row, result, index) {
    if (!row.name) {
...
async function upsertPayment(tx, row, result, index) {
...
async function upsertLedgerEntry(tx, row, result, index) {
