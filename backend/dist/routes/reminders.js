...
const reminderService_1 = require("../services/reminder-engine/services/reminderService");
...
router.post('/', requireAuthOrDevBypass, requirePermissionOrDevBypass('reminder.create'), async (req, res) => {
    const userId = req.user?.id || 'dev-user';
    const body = req.body || {};
    const reminderDate = new Date(String(body.reminder_date || body.reminderDate));
    const reminderTime = new Date(`1970-01-01T${String(body.reminder_time || body.reminderTime || '00:00')}:00.000Z`);
    const triggerTime = (0, reminderService_1.computeTriggerTime)(reminderDate, reminderTime);
    const reminder = await client_1.default.reminder.create({
        data: {
...
router.get('/', requireAuthOrDevBypass, requirePermissionOrDevBypass('reminder.view'), async (req, res) => {
    const userId = req.user?.id;
    const { module_name, record_id } = req.query;
    const reminders = await client_1.default.reminder.findMany({
        where: module_name
...
router.get('/:id', requireAuthOrDevBypass, requirePermissionOrDevBypass('reminder.view'), async (req, res) => {
    const reminder = await client_1.default.reminder.findUnique({ where: { id: String(req.params.id) } });
    if (!reminder)
        return res.status(404).json({ error: 'Not found' });
    res.json({ data: reminder });
});
router.put('/:id', requireAuthOrDevBypass, requirePermissionOrDevBypass('reminder.update'), async (req, res) => {
    const body = req.body || {};
    const updatedReminder = await client_1.default.reminder.update({
        ...
        data: {
            ...
            assignedToUser: body.assigned_to_user != null ? String(body.assigned_to_user) : undefined,
    ...
    if (body.reminder_date || body.reminder_time) {
        ...
        const triggerTime = (0, reminderService_1.computeTriggerTime)(effectiveDate, effectiveTime);
...
router.delete('/:id', requireAuthOrDevBypass, requirePermissionOrDevBypass('reminder.delete'), async (req, res) => {
    await client_1.default.reminder.delete({ where: { id: String(req.params.id) } });
    res.status(204).send();
});
