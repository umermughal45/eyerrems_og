...
router.put('/', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        const { companyName, companyEmail, supportPhone, companyAddress, companyLogo, selectedCurrency, notificationConfig, integrationConfig } = req.body;
        // Validation
        ...
        if (selectedCurrency !== undefined)
            updateData.selectedCurrency = selectedCurrency;
        if (notificationConfig !== undefined)
            updateData.notificationConfig = notificationConfig;
        if (integrationConfig !== undefined) {
            ...
            if (integrationConfig.resend?.apiKey && integrationConfig.resend.apiKey.includes('...')) {
                const current = await settings_service_1.settingsService.getSettings();
        ...
        const updated = await settings_service_1.settingsService.updateSettings(updateData, req);
 ...
 */
router.post('/clear-cache', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        settings_service_1.settingsService.clearCache();
 ...
 */
router.post('/generate-report', auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        logger_1.default.info(`System report requested by ${req.user?.username || req.user?.email}`);
        const reportData = await settings_service_1.settingsService.generateFullSystemReport(req);
        res.json({
            success: true,
