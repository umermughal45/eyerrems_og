...
router.get('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const { AttendanceStateService } = await Promise.resolve().then(() => __importStar(require('../services/attendance-state-service')));
...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
        const attendance = await client_1.default.attendance.findUnique({
            where: { id },
            include: {
                employee: {
                    select: {
...
router.post('/', auth_1.authenticate, async (req, res) => {
    try {
        ...
        const existingAttendance = await client_1.default.attendance.findFirst({
            where: {
                employeeId,
...
// Check-in
router.post('/checkin', auth_1.authenticate, async (req, res) => {
    try {
        ...
        let attendance = await client_1.default.attendance.findFirst({
            where: {
                employeeId,
                date: { gte: today, lte: endOfDay },
                isDeleted: false,
        ...
        if (attendance) {
            ...
            attendance = await client_1.default.attendance.update({
            ...
            try {
                ...
                attendance = await client_1.default.attendance.create({
                    data: {
                        employeeId,
                        date: today,
                ...
                if (error.code === 'P2002') {
                    ...
                    attendance = await client_1.default.attendance.findFirst({
                        where: {
                            employeeId,
                            date: { gte: today, lte: endOfDay },
                        },
...
router.get('/employee/:id', auth_1.authenticate, async (req, res) => {
    try {
        const { id } = req.params;
