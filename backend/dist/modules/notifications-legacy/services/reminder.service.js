...
class ReminderService {
    constructor(pool) {
        this.pool = pool;
        this.notificationService = new notification_service_1.NotificationService(pool);
    }
    async createReminder(input, createdBy) {
    ...
    `;
        ...
        if (input.notification_channel) {
            const channel = input.notification_channel;
            const title = input.notification_title ?? `Reminder: ${reminder.title}`;
            const message = input.notification_message ??
            ...
            if (input.scheduled_notification_at) {
                await this.notificationService.scheduleNotification({
                    ...
                    title,
                    message,
                }, input.scheduled_notification_at, createdBy);
            }
            else {
    ...
    async updateReminder(id, input, updatedBy) {
    ...
    async completeReminder(id, updatedBy) {
        return this.updateReminder(id, { status: 'completed' }, updatedBy);
    }
    async getUserReminders(userId, opts) {
    ...
    async getRemindersByModule(moduleName, recordId, opts) {
