...
class LedgerService {
     ...
     */
    static async createLedgerEntry(payload, tx) {
     ...
     */
    static async getAccountByAlias(alias) {
        const accountAlias = await client_1.default.accountAlias.findUnique({
            where: { alias },
        ...
        const account = await client_1.default.account.findFirst({
            where: {
     ...
     */
    static async getClientLedger(clientId, filters) {
        const where = {
            deletedAt: null,
        };
        if (clientId) {
     ...
     */
    static async getPropertyLedger(propertyId) {
        const where = {
            deletedAt: null,
        };
        if (propertyId) {
    ...
    static async getCompanyLedger(filters) {
        ...
        if (filters?.startDate || filters?.endDate) {
            ...
            if (filters.startDate)
                journalEntryWhere.date.gte = filters.startDate;
            if (filters.endDate)
                journalEntryWhere.date.lte = filters.endDate;
        }
        // Build JournalLine filter
        const journalLineWhere = {};
    ...
    static async calculateAccountBalances(filters) {
        ...
        const cashAccount = accounts.find((a) => a.code?.startsWith('1111') ||
            a.code?.startsWith('111101') ||
            a.code?.startsWith('111102') ||
            a.name?.toLowerCase().includes('cash'));
        const bankAccount = accounts.find((a) => a.code?.startsWith('1112') ||
            a.code?.startsWith('111201') ||
            a.code?.startsWith('111202') ||
            a.name?.toLowerCase().includes('bank'));
        const arAccount = accounts.find((a) => a.code?.startsWith('113') ||
            a.name?.toLowerCase().includes('receivable'));
        const dealerPayable = accounts.find((a) => a.code?.startsWith('211') ||
