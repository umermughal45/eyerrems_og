...
class MailService {
    ...
    async sendEmail(to, subject, html, text) {
        try {
            ...
            if (response.error && (response.error.message.toLowerCase().includes('domain') || response.error.message.toLowerCase().includes('verified'))) {
                logger_1.default.warn(`Domain for ${senderEmail} may not be verified in Resend. Falling back to onboarding@resend.dev and setting Reply-To.`);
            ...
            logger_1.default.info(`Email sent via Resend: ${response.data?.id}`);
            // Persist the sent email into the application database natively
            await client_1.default.emailMessage.create({
                data: {
                    to,
                    subject,
                    body: html,
            ...
            const internalUsers = await client_1.default.user.findMany({ select: { email: true } });
            const internalEmails = internalUsers.map(u => u.email.toLowerCase());
            const isInternalRecipient = internalEmails.includes(to.toLowerCase()) ||
                (settings.companyEmail && to.toLowerCase() === settings.companyEmail.toLowerCase());
            if (isInternalRecipient) {
                logger_1.default.info(`Routing internal email directly to Inbox for: ${to}`);
                await client_1.default.emailMessage.create({
                    data: {
                        to,
                        subject,
            ...
            logger_1.default.error('Error sending email via Resend:', error);
            throw error;
        }
    }
    /**
    ...
    async fetchInbox(limit = 20) {
        try {
            const messages = await client_1.default.emailMessage.findMany({
            ...
            logger_1.default.error('Error fetching inbox messages from DB:', error);
            throw error;
        }
    }
    /**
    ...
    async fetchSent(limit = 20) {
        try {
            ...
            logger_1.default.error('Error fetching sent messages from DB:', error);
            throw error;
        }
    }
    /**
    ...
    async handleWebhook(payload) {
        try {
            ...
            logger_1.default.error('Error processing mail webhook:', error);
            throw error;
        }
    }
    /**
    ...
    async getUnreadCount() {
        try {
            const count = await client_1.default.emailMessage.count({
                where: { direction: 'received', isRead: false }
            });
            return count;
        }
        catch (error) {
            logger_1.default.error('Error fetching unread count from DB:', error);
            return 0;
        }
    }
    /**
    ...
    async getMessage(uid) {
        try {
            const message = await client_1.default.emailMessage.findUnique({
                where: { id: uid }
            });
            if (!message) {
            ...
            if (message.direction === 'received' && !message.isRead) {
                await client_1.default.emailMessage.update({
                    where: { id: uid },
                    data: { isRead: true }
                });
            }
            ...
            logger_1.default.error('Error fetching message body from DB:', error);
            throw error;
        }
    }
    /**
    ...
    async syncInbox() {
        try {
            ...
            logger_1.default.info('IMAP client connected successfully for sync.');
            ...
            try {
                ...
                if (total > 0) {
                    ...
                    for await (let msg of client.fetch(fetchQuery, { source: true, uid: true })) {
                        ...
                        let to = parsed.to?.value.map((val) => val.address).join(', ') || imapConfig.user;
                        ...
                        const existing = await client_1.default.emailMessage.findFirst({
                            where: {
                                direction: 'received',
                                subject: subject,
                        ...
                        if (!existing) {
                            const newMsg = await client_1.default.emailMessage.create({
                                data: {
                                    to,
                                    subject,
                                    body,
            ...
            logger_1.default.error('Error syncing IMAP inbox:', error);
