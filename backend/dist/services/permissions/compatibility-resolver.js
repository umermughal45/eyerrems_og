...
exports.convertLegacyPermissions = convertLegacyPermissions;
 ...
 */
async function hasExplicitPermissions(roleId) {
    const count = await client_1.default.rolePermission.count({
        where: { roleId },
    });
    return count > 0;
}
 ...
 */
async function convertLegacyPermissions(roleId, legacyPermissions, actorId = 'system') {
    ...
    if (hasExplicit) {
        logger_1.default.info(`Role ${role.name} already has explicit permissions, skipping conversion`);
        return;
    }
    logger_1.default.info(`Converting legacy permissions for role: ${role.name}`);
    ...
    if (legacyPermissions.includes('*') || legacyPermissions.includes('admin.*')) {
        ...
        for (const [module, permissions] of Object.entries(allPermissions)) {
            for (const permission of permissions) {
                ...
                if (parts.length === 2) {
                    await (0, permission_service_1.grantPermission)(roleId, parts[0], undefined, parts[1], actorId);
                }
                else if (parts.length === 3) {
                    await (0, permission_service_1.grantPermission)(roleId, parts[0], parts[1], parts[2], actorId);
                }
            }
        }
        logger_1.default.info(`Converted Admin role: granted ${Object.values(allPermissions).flat().length} explicit permissions`);
        return;
    }
    ...
    for (const legacyPerm of legacyPermissions) {
        ...
        if (parts.length === 2) {
            ...
            await (0, permission_service_1.grantPermission)(roleId, module, undefined, action, actorId);
            ...
            if (action === 'view') {
                ...
                for (const submodule of commonSubmodules) {
                    try {
                        await (0, permission_service_1.grantPermission)(roleId, module, submodule, action, actorId);
                    }
                    catch (e) {
    ...
    logger_1.default.info(`Converted legacy permissions for role ${role.name}: ${legacyPermissions.length} → explicit grants`);
}
 ...
 */
async function resolveRolePermissions(roleId, legacyPermissions) {
    ...
    if (!hasExplicit) {
        ...
        if (existingLock) {
            ...
            const conversionPromise = (async () => {
                try {
                    logger_1.default.info(`Auto-converting legacy permissions for role ${roleId}`);
    ...
    if (roleStatus === 'DEACTIVATED') {
        logger_1.default.info(`Role ${role.name} (${roleId}) is DEACTIVATED - returning empty permissions`);
        return [];
    }
...
//# sourceMappingURL=compatibility-resolver.js.map