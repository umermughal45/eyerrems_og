...
exports.validateManualUniqueId = validateManualUniqueId;
exports.validateTID = validateTID;
exports.generatePrefixedId = generatePrefixedId;
exports.parseSystemId = parseSystemId;
...
async function generateSequenceNumber(prefix) {
    try {
        const seq = await client_1.default.sequence.update({
            where: { prefix },
        ...
        if (error.code === 'P2025') {
            ...
            const seq = await client_1.default.sequence.upsert({
                where: { prefix },
...
async function findMaxSystemId(modulePrefix, prefix, client) {
    let maxCounter = 0;
 ...
 */
async function generateSystemId(modulePrefix, tx) {
 ...
 */
async function validateManualUniqueId(manualId, modulePrefix, excludeId, tx) {
 ...
 * @returns true if valid, throws error if invalid
 */
async function validateTID(tid, excludePropertyId, excludeDealId, excludeClientId, excludeLeadId, excludeEmployeeId, excludeTenantId, tx) {
    if (!tid || tid.trim() === '') {
    ...
    if (propertyTidExists) {
        queries.push(client.property.findFirst({
            where: {
                tid: trimmedTid,
    ...
    if (dealTidExists) {
        queries.push(client.deal.findFirst({
            where: {
                tid: trimmedTid,
                isDeleted: false,
                deletedAt: null,
    ...
    if (clientTidExists) {
        queries.push(client.client.findFirst({
            where: {
                tid: trimmedTid,
                isDeleted: false,
    ...
    if (leadTidExists) {
        queries.push(client.lead.findFirst({
            where: {
                tid: trimmedTid,
                isDeleted: false,
    ...
    if (employeeTidExists) {
        queries.push(client.employee.findFirst({
            where: {
                tid: trimmedTid,
                isDeleted: false,
    ...
    if (tenantTidExists) {
        queries.push(client.tenant.findFirst({
            where: {
                tid: trimmedTid,
                isDeleted: false,
 ...
 */
async function generatePrefixedId(customPrefix, entityType, tx) {
