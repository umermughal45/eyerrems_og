...
const client_property_ledger_service_1 = require("./client-property-ledger-service");
class UnifiedLedgerService {
    /**
     * Get unified ledger for any entity type
     */
    static async getLedger(type, id, filters) {
        switch (type) {
            case 'client':
    ...
    static async getClientLedger(clientId, filters) {
        const res = await (0, client_property_ledger_service_1.getClientLedger)(clientId, filters);
        ...
        const entries = allEntries.map((e) => ({
            ...
            sourceType: e.source_type.toLowerCase(),
    ...
    static async getDealerLedger(dealerId, filters) {
        ...
        try {
            const engineEntries = await (0, ledger_engine_service_1.getLedgerEntries)('dealer', dealerId, filters);
     ...
     */
    static async getPropertyLedger(propertyId, filters) {
        const res = await (0, client_property_ledger_service_1.getPropertyLedger)(propertyId, filters);
        const allEntries = res.openingBalanceRow
            ? [{ ...res.openingBalanceRow, running_balance: 0 }, ...res.entries]
            : res.entries;
        const entries = allEntries.map((e) => ({
