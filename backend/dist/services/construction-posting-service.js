...
exports.ConstructionPostingService = void 0;
const client_1 = __importDefault(require("../prisma/client"));
const account_validation_service_1 = require("./account-validation-service");
const logger_1 = __importDefault(require("../utils/logger"));
class ConstructionPostingService {
    /**
     * Get posting rule for an event type
     */
    static async getPostingRule(eventType) {
        const rule = await client_1.default.constructionPostingRule.findUnique({
            where: { eventType },
        });
     ...
     * CR: Inventory
     */
    static async postMaterialIssue(issueId, dimensions, amount, userId) {
        ...
        await account_validation_service_1.AccountValidationService.validateAccountPostable(debitAccountId);
        await account_validation_service_1.AccountValidationService.validateAccountPostable(creditAccountId);
        // Create journal entry
        const entryNumber = this.generateEntryNumber('JV');
        const voucherNo = this.generateVoucherNumber('CONST');
        const journalEntry = await client_1.default.journalEntry.create({
            data: {
                entryNumber,
                voucherNo,
                date: new Date(),
        ...
        await client_1.default.constructionIssue.update({
            where: { id: issueId },
            data: { journalEntryId: journalEntry.id, status: 'posted' },
        });
        logger_1.default.info(`Material Issue ${issueId} posted to Finance: ${journalEntry.id}`);
     ...
     * CR: Payroll Accrual
     */
    static async postLaborApproval(laborId, dimensions, amount, userId) {
        this.validateDimensions(dimensions);
        const project = await client_1.default.constructionProject.findUnique({
            where: { id: dimensions.projectId },
        });
        ...
        await client_1.default.constructionLabor.update({
            where: { id: laborId },
            data: { journalEntryId: journalEntry.id, status: 'posted' },
        });
        logger_1.default.info(`Labor Approval ${laborId} posted to Finance: ${journalEntry.id}`);
        return journalEntry.id;
    }
    /**
     ...
     */
    static async postEquipmentUsage(usageId, dimensions, amount, userId) {
        this.validateDimensions(dimensions);
        const project = await client_1.default.constructionProject.findUnique({
            where: { id: dimensions.projectId },
        });
        ...
        await client_1.default.constructionEquipmentUsage.update({
            where: { id: usageId },
            data: { journalEntryId: journalEntry.id, status: 'posted' },
        });
        logger_1.default.info(`Equipment Usage ${usageId} posted to Finance: ${journalEntry.id}`);
        return journalEntry.id;
    }
    /**
     ...
     * CR: Accounts Payable
     */
    static async postSubcontractorInvoice(invoiceId, dimensions, amount, userId) {
        this.validateDimensions(dimensions);
        const project = await client_1.default.constructionProject.findUnique({
            where: { id: dimensions.projectId },
        });
        ...
        logger_1.default.info(`Subcontractor Invoice ${invoiceId} posted to Finance: ${journalEntry.id}`);
        return journalEntry.id;
    }
    /**
     ...
     */
    static async postClientBilling(milestoneId, dimensions, billingAmount, retentionAmount, userId) {
        this.validateDimensions(dimensions);
        const project = await client_1.default.constructionProject.findUnique({
            where: { id: dimensions.projectId },
        });
        ...
        await account_validation_service_1.AccountValidationService.validateAccountPostable(revenueAccountId);
        await account_validation_service_1.AccountValidationService.validateAccountPostable(arAccountId);
        if (retentionAccountId) {
            await account_validation_service_1.AccountValidationService.validateAccountPostable(retentionAccountId);
        }
        const entryNumber = this.generateEntryNumber('JV');
        const voucherNo = this.generateVoucherNumber('CONST');
        const lines = [
        ...
        logger_1.default.info(`Client Billing ${milestoneId} posted to Finance: ${journalEntry.id}`);
        return journalEntry.id;
    }
    /**
     ...
     * CR: WIP
     */
    static async postProjectClose(projectId, wipAmount, userId) {
        const project = await client_1.default.constructionProject.findUnique({
            where: { id: projectId },
        });
        if (!project) {
        ...
        await account_validation_service_1.AccountValidationService.validateAccountPostable(cogsAccountId);
        await account_validation_service_1.AccountValidationService.validateAccountPostable(wipAccountId);
        const entryNumber = this.generateEntryNumber('JV');
        const voucherNo = this.generateVoucherNumber('CONST');
        const journalEntry = await client_1.default.journalEntry.create({
            data: {
        ...
        logger_1.default.info(`Project Close ${projectId} posted to Finance: ${journalEntry.id}`);
        return journalEntry.id;
    }
    /**
     * Generate journal entry number
     */
    static generateEntryNumber(prefix) {
        const date = new Date();
     ...
     */
    static generateVoucherNumber(prefix) {
        const date = new Date();
        const year = date.getFullYear();
        const month = `${date.getMonth() + 1}`.padStart(2, '0');
        const day = `${date.getDate()}`.padStart(2, '0');
...
exports.ConstructionPostingService = ConstructionPostingService;
//# sourceMappingURL=construction-posting-service.js.map