...
class FinancialReportingService {
     ...
     */
    static async calculateAccountBalance(accountId, startDate, endDate) {
        const where = {
            OR: [
     ...
     */
    static async calculateAccountBalanceWithChildren(accountId, startDate, endDate) {
        const account = await client_1.default.account.findUnique({
            where: { id: accountId },
            include: {
                children: {
        ...
        if (account.children && account.children.length > 0) {
            ...
            for (const child of account.children) {
                const childBalance = await this.calculateAccountBalanceWithChildren(child.id, startDate, endDate);
     ...
     */
    static async getAllDescendantAccounts(accountId) {
        const account = await client_1.default.account.findUnique({
            where: { id: accountId },
            include: {
                children: {
    ...
    static async generateTrialBalance(startDate, endDate) {
        ...
        for (const account of accounts) {
            ...
            if (account.isPostable && account.level === 5) {
                ...
                const aggregated = await this.calculateAccountBalanceWithChildren(account.id, startDate, endDate);
    ...
    static async generateBalanceSheet(asOfDate) {
        ...
        const allAccounts = await client_1.default.account.findMany({
            where: { isActive: true },
            include: {
                children: {
                    where: { isActive: true },
        ...
        const assetAccounts = allAccounts.filter(a => a.code.startsWith('1') && !a.trustFlag && a.type === 'Asset');
        const currentAssetAccounts = assetAccounts.filter(a => isCurrentAsset(a.code) || (a.parent && isCurrentAsset(a.parent.code)));
        const fixedAssetAccounts = assetAccounts.filter(a => isFixedAsset(a.code) || (a.parent && isFixedAsset(a.parent.code)));
        // Liabilities (2xxx) - current liabilities (21xx)
        const liabilityAccounts = allAccounts.filter(a => a.code.startsWith('2') &&
            (a.isPostable || (a.children && a.children.length > 0)) &&
            !a.trustFlag);
        // Equity (3xxx)
        const equityAccounts = allAccounts.filter(a => a.code.startsWith('3') &&
            (a.isPostable || (a.children && a.children.length > 0)));
        ...
        const capitalEquity = equity.filter(e => e.accountCode.startsWith('3100') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('3100')));
        const retainedEarningsEquity = equity.filter(e => e.accountCode.startsWith('3200') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('3200')));
        const currentYearProfitEquity = equity.filter(e => e.accountCode.startsWith('3300') ||
     ...
     */
    static async generateProfitAndLoss(startDate, endDate) {
        // Get all accounts with hierarchy
        const allAccounts = await client_1.default.account.findMany({
            where: { isActive: true },
            include: {
        ...
        const propertyRevenue = revenueEntries.filter(e => e.accountCode.startsWith('411') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('411')));
        const serviceIncome = revenueEntries.filter(e => e.accountCode.startsWith('421') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('421')));
        // Categorize expenses by code prefixes
        const sellingExpenses = expenseEntries.filter(e => e.accountCode.startsWith('511') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('511')));
        const propertyExpenses = expenseEntries.filter(e => e.accountCode.startsWith('521') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('521')));
        const administrativeExpenses = expenseEntries.filter(e => e.accountCode.startsWith('531') ||
            (allAccounts.find(a => a.id === e.accountId)?.parent?.code?.startsWith('531')));
        const taxExpenses = expenseEntries.filter(e => e.accountCode.startsWith('541') ||
     ...
     */
    static async generatePropertyProfitability(propertyId, startDate, endDate) {
        // Build date filter
        const dateFilter = {};
        if (startDate)
        ...
        const financeLedgerWhere = {
            isDeleted: false,
            ...(propertyId ? { propertyId } : {}),
            ...(Object.keys(dateFilter).length > 0 && { date: dateFilter }),
        };
        const financeLedgerEntries = await client_1.default.financeLedger.findMany({
        ...
        for (const entry of ledgerEntries) {
            ...
            if (propertyId && deal.property.id !== propertyId)
                continue;
            ...
            if (!profitability)
                continue; // Skip if property not found in map
            ...
            if (entry.debitAccount && entry.debitAccount.type === 'Expense' && entryAmount !== 0) {
                ...
                const existing = profitability.expenseBreakdown.find(e => e.accountCode === entry.debitAccount.code);
                if (existing) {
            ...
            if (entry.creditAccount && entry.creditAccount.type === 'Revenue' && entryAmount !== 0) {
                ...
                const existing = profitability.revenueBreakdown.find(r => r.accountCode === entry.creditAccount.code);
                if (existing) {
                    existing.amount += entryAmount;
                }
                else {
        ...
        for (const entry of financeLedgerEntries) {
            ...
            if (entry.category === 'Income' || entry.category === 'Revenue') {
                profitability.revenue += entryAmount;
                const existing = profitability.revenueBreakdown.find(r => r.accountCode === entry.category);
                if (existing) {
                    existing.amount += entryAmount;
                }
                else {
                ...
                profitability.expenses += entryAmount;
                const existing = profitability.expenseBreakdown.find(e => e.accountCode === entry.category);
                if (existing) {
                    existing.amount += entryAmount;
                }
                else {
     ...
     */
    static async generateEscrowReport() {
        // Get all trust asset accounts (Asset accounts with trustFlag = true)
        const trustAssetAccounts = await client_1.default.account.findMany({
            where: {
                isActive: true,
                trustFlag: true,
                type: 'Asset',
        ...
        const clientLiabilityAccounts = await client_1.default.account.findMany({
            where: {
                isActive: true,
                trustFlag: true,
                type: 'Liability',
        ...
        const trustAssets = await Promise.all(trustAssetAccounts.map(async (account) => {
        ...
        const clientLiabilities = await Promise.all(clientLiabilityAccounts.map(async (account) => {
            const balances = await this.calculateAccountBalanceWithChildren(account.id);
            return {
                accountCode: account.code,
                accountName: account.name,
        ...
        const violations = [];
        if (!isBalanced) {
            violations.push(`Escrow balance mismatch: Trust Assets (${totalTrustAssets.toFixed(2)}) ≠ Client Liabilities (${totalClientLiabilities.toFixed(2)})`);
        }
     ...
     */
    static async generateAgingReport(type, asOfDate) {
        ...
        for (const account of accounts) {
            ...
            if (!isPostingAccount && !hasPostingChildren)
                continue;
            ...
            const balanceByDate = new Map(); // Map of timestamp to balance
            ...
            for (const timestamp of sortedDates) {
                ...
                const daysDiff = Math.floor((cutoffDate.getTime() - timestamp) / (1000 * 60 * 60 * 24));
                ...
                if (daysDiff <= 30) {
                    ...
                    const olderThan60 = balanceByDate.get(sortedDates.find(d => Math.floor((cutoffDate.getTime() - d) / (1000 * 60 * 60 * 24)) > 60) || timestamp) || 0;
                    days31_60 = Math.max(0, balance - olderThan60);
                }
                else if (daysDiff <= 90) {
                    const olderThan90 = balanceByDate.get(sortedDates.find(d => Math.floor((cutoffDate.getTime() - d) / (1000 * 60 * 60 * 24)) > 90) || timestamp) || 0;
            ...
            if (finalBalance > 0) {
                ...
                if (oldestDate) {
                    const oldestDays = Math.floor((cutoffDate.getTime() - oldestDate.getTime()) / (1000 * 60 * 60 * 24));
                    if (oldestDays <= 30) {
