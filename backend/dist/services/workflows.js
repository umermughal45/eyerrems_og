...
exports.syncInvoiceToFinanceLedger = syncInvoiceToFinanceLedger;
exports.syncPaymentToFinanceLedger = syncPaymentToFinanceLedger;
exports.syncPropertyExpenseToFinanceLedger = syncPropertyExpenseToFinanceLedger;
exports.syncMaintenanceToFinanceLedger = syncMaintenanceToFinanceLedger;
exports.syncCommissionToFinanceLedger = syncCommissionToFinanceLedger;
exports.syncDealToFinanceLedger = syncDealToFinanceLedger;
exports.syncPayrollToFinanceLedger = syncPayrollToFinanceLedger;
exports.updateTenantLedger = updateTenantLedger;
exports.updateDashboardKPIs = updateDashboardKPIs;
exports.createTenancyFromLease = createTenancyFromLease;
exports.updatePropertyStatusOnMaintenance = updatePropertyStatusOnMaintenance;
...
async function generateMonthlyInvoices() {
    ...
    const tenancies = await client_1.default.tenancy.findMany({
        where: {
            status: 'active',
            OR: [
    ...
    for (const tenancy of tenancies) {
        try {
            ...
            await client_1.default.tenancy.update({
                where: { id: tenancy.id },
 ...
 */
async function syncInvoiceToFinanceLedger(invoiceId) {
 ...
 */
async function syncPaymentToFinanceLedger(paymentId) {
    const payment = await client_1.default.tenantPayment.findUnique({
        where: { id: paymentId },
        include: {
            tenant: true,
    ...
    if (payment.tenantId && revenueAmount > 0) {
        await updateTenantLedger(payment.tenantId, {
            entryType: 'credit',
            description: `Payment received ${payment.paymentId}${payment.allocations?.length ? ` (${payment.allocations.length} invoice(s))` : ''}`,
    ...
    const ledger = await client_1.default.financeLedger.create({
        data: {
            ...
            propertyId: payment.invoice?.propertyId || payment.propertyId || null,
 ...
 */
async function syncPropertyExpenseToFinanceLedger(expenseId) {
    const expense = await client_1.default.propertyExpense.findUnique({
        where: { id: expenseId },
    ...
    await client_1.default.propertyExpense.update({
        where: { id: expenseId },
 ...
 */
async function syncMaintenanceToFinanceLedger(maintenanceId) {
    const maintenance = await client_1.default.maintenanceRequest.findUnique({
    ...
    await client_1.default.maintenanceRequest.update({
        where: { id: maintenanceId },
        data: { financeLedgerId: ledger.id },
    });
    return ledger;
 ...
 */
async function syncCommissionToFinanceLedger(commissionId) {
    ...
    const deal = await client_1.default.deal.findFirst({
        where: {
 ...
 */
async function syncDealToFinanceLedger(dealId) {
    ...
    if (deal.dealerId && deal.commissionRate > 0 && deal.commissionAmount > 0) {
        ...
        const existingCommission = await client_1.default.commission.findFirst({
            where: {
        ...
        if (!existingCommission) {
            await client_1.default.commission.create({
                data: {
                    dealerId: deal.dealerId,
            ...
            const commission = await client_1.default.commission.findFirst({
                where: {
                    dealerId: deal.dealerId,
                    saleId: null,
                },
 ...
 */
async function syncPayrollToFinanceLedger(payrollId) {
 ...
 */
async function updateTenantLedger(tenantId, entry) {
    // Get current balance
    const lastEntry = await client_1.default.tenantLedger.findFirst({
        where: { tenantId, isDeleted: false },
        orderBy: { createdAt: 'desc' },
    });
    const currentBalance = lastEntry?.balance || 0;
    const newBalance = entry.entryType === 'debit'
    ...
    // Create ledger entry
    const ledgerEntry = await client_1.default.tenantLedger.create({
        data: {
 ...
 */
async function updateDashboardKPIs(propertyId) {
    try {
        ...
        await client_1.default.revenueSummary.upsert({
            where: {
                propertyId_month: {
                    propertyId,
                    month: currentMonth,
 ...
 */
async function createTenancyFromLease(leaseId) {
    ...
    const tenancy = await client_1.default.tenancy.create({
        data: {
 ...
 */
async function updatePropertyStatusOnMaintenance(maintenanceId) {
    const maintenance = await client_1.default.maintenanceRequest.findUnique({
        where: { id: maintenanceId },
