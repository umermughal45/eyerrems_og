...
exports.DealSafetyService = void 0;
const client_1 = __importDefault(require("../prisma/client"));
class DealSafetyService {
    /**
     ...
     */
    static async validateDealCreation(payload) {
        ...
        if (!payload.dealAmount || payload.dealAmount <= 0) {
            throw new Error('DEAL_VIOLATION: Deal amount must be greater than zero. ' +
        ...
        if (!client) {
            throw new Error('DEAL_VIOLATION: Client not found or inactive. ' +
        ...
        if (payload.propertyId) {
            ...
            if (!property) {
                throw new Error('DEAL_VIOLATION: Property not found or inactive. ' +
     ...
     */
    static async validateDealUpdate(payload) {
        ...
        if ((deal.status === 'closed' || deal.stage === 'closed-won') &&
            ...
            throw new Error('DEAL_VIOLATION: Closed deals are read-only. ' +
        ...
        if (deal.status === 'cancelled' || deal.stage === 'closed-lost') {
            if (payload.clientId || payload.propertyId || payload.dealAmount !== undefined || payload.stage) {
                throw new Error('DEAL_VIOLATION: Cancelled deals cannot be modified. ' +
     ...
     */
    static async validateClientPropertyImmutability(dealId, field) {
        // Check for linked payments
        const paymentCount = await client_1.default.payment.count({
            where: {
                dealId,
                deletedAt: null,
            },
        ...
        if (deal?.propertyId) {
            ...
            invoiceCount = await client_1.default.invoice.count({
                where: {
        ...
        if (paymentCount > 0 || invoiceCount > 0) {
            ...
            if (paymentCount > 0)
                linkedEntities.push(`${paymentCount} payment(s)`);
            if (invoiceCount > 0)
                linkedEntities.push(`${invoiceCount} invoice(s)`);
            throw new Error(`DEAL_VIOLATION: ${field === 'client' ? 'Client' : 'Property'} cannot be changed. ` +
     ...
     */
    static async validateDealAmountReduction(dealId, newAmount, currentAmount) {
        ...
        if (totalInvoiceAmount > newAmount) {
            throw new Error(`DEAL_VIOLATION: Cannot reduce deal amount. ` +
     ...
     */
    static async validateStageChange(payload) {
        const deal = await client_1.default.deal.findUnique({
            where: { id: payload.dealId },
            include: {
                payments: {
        ...
        if (payload.newStage === 'prospecting') {
            if (deal.payments.length > 0) {
                throw new Error('DEAL_VIOLATION: Cannot move deal to Prospecting stage. ' +
        ...
        if (payload.newStage === 'negotiation') {
            const nonAdvancePayments = deal.payments.filter((p) => p.paymentType !== 'token' && p.paymentType !== 'booking');
            if (nonAdvancePayments.length > 0) {
                throw new Error('DEAL_VIOLATION: Cannot move deal to Negotiation stage. ' +
     ...
     */
    static async validateCancellation(payload) {
        const deal = await client_1.default.deal.findUnique({
            where: { id: payload.dealId },
            include: {
                payments: {
        ...
        if (postedInvoiceCount > 0 || unreversedPaymentCount > 0) {
            ...
            throw new Error(`DEAL_VIOLATION: Cannot cancel deal. Deal has linked financial records: ${reasons.join(', ')}. ` +
        ...
        if (!payload.reason || !payload.reason.trim()) {
            throw new Error('DEAL_VIOLATION: Cancellation reason is required. ' +
     ...
     */
    static async validateClosure(dealId) {
        const deal = await client_1.default.deal.findUnique({
            where: { id: dealId },
        ...
        if (deal.stage === 'closing' && deal.propertyId) {
            const invoiceCount = await client_1.default.invoice.count({
                where: {
                    propertyId: deal.propertyId,
                    status: {
                        notIn: ['cancelled'],
            ...
            if (invoiceCount === 0) {
                ...
                console.warn(`DEAL_VIOLATION_WARNING: Deal ${dealId} is being closed without invoices. ` +
                    'Consider creating invoices before closing the deal.');
            }
        }
    }
}
exports.DealSafetyService = DealSafetyService;
//# sourceMappingURL=deal-safety-service.js.map