...
class AccountingSafetyService {
    ...
    static async validateInvoiceCreation(payload) {
        ...
        if (!payload.tenantId) {
            throw new Error('ACCOUNTING_VIOLATION: Tenant/Customer is required for invoice creation. ' +
        ...
        if (!payload.amount || payload.amount <= 0) {
            throw new Error('ACCOUNTING_VIOLATION: Invoice amount must be greater than zero. ' +
                `Received: ${payload.amount}. Zero-value invoices are not allowed.`);
        }
        if (!payload.totalAmount || payload.totalAmount <= 0) {
            throw new Error('ACCOUNTING_VIOLATION: Invoice total amount must be greater than zero. ' +
        ...
        if (payload.dealId) {
            ...
            if (!deal) {
                throw new Error(`ACCOUNTING_VIOLATION: Deal not found: ${payload.dealId}. ` +
                    'Invoice cannot be created for non-existent deal.');
            }
            if (deal.status !== 'Approved' && deal.status !== 'Active') {
                throw new Error(`ACCOUNTING_VIOLATION: Deal must be Approved or Active to create invoice. ` +
        ...
        if (payload.tenantAccountId) {
            await this.validateAccountTypeForInvoice(payload.tenantAccountId, 'debit', 'Accounts Receivable');
        }
        if (payload.incomeAccountId) {
            await this.validateAccountTypeForInvoice(payload.incomeAccountId, 'credit', 'Revenue');
        }
        ...
        if (payload.tenantAccountId && payload.incomeAccountId) {
            const tenantAccount = await client_1.default.account.findUnique({
                where: { id: payload.tenantAccountId },
                select: { type: true, code: true, name: true },
            });
            const incomeAccount = await client_1.default.account.findUnique({
                where: { id: payload.incomeAccountId },
                select: { type: true, code: true, name: true },
            });
            if (tenantAccount && !this.isAccountsReceivableAccount(tenantAccount)) {
                throw new Error(`ACCOUNTING_VIOLATION: Tenant account must be Accounts Receivable type. ` +
            ...
            if (incomeAccount && incomeAccount.type !== 'Revenue') {
                throw new Error(`ACCOUNTING_VIOLATION: Income account must be Revenue type. ` +
     ...
     */
    static async validateAccountTypeForInvoice(accountId, side, expectedType) {
        ...
        if (isCashBank) {
            throw new Error(`ACCOUNTING_VIOLATION: Cash/Bank accounts cannot be used in invoice creation. ` +
        ...
        if (side === 'credit' && account.type === 'Asset' && !this.isAccountsReceivableAccount(account)) {
            throw new Error(`ACCOUNTING_VIOLATION: Asset accounts (except Accounts Receivable) cannot be credited in invoices. ` +
                `Received: ${account.code} - ${account.name}. ` +
                'Invoice credit must be Revenue / Tax Payable ONLY.');
        }
        // DISALLOW: Equity accounts in operational flows
        if (account.type === 'Equity') {
            throw new Error(`ACCOUNTING_VIOLATION: Equity accounts cannot be used in invoice creation. ` +
                `Received: ${account.code} - ${account.name}. ` +
                'Equity accounts are restricted from operational flows.');
        }
        // Validate account is postable
        await account_validation_service_1.AccountValidationService.validateAccountPostable(accountId);
    }
    /**
     * Check if account is Accounts Receivable type
     */
    static isAccountsReceivableAccount(account) {
    ...
    static async validatePaymentCreation(payload) {
        ...
        if (!payload.dealId) {
            throw new Error('ACCOUNTING_VIOLATION: Deal is required for payment recording. ' +
                'Payments must be linked to a deal for proper lifecycle tracking.');
        }
        // BLOCK: Amount ≤ 0
        if (!payload.amount || payload.amount <= 0) {
            throw new Error('ACCOUNTING_VIOLATION: Payment amount must be greater than zero. ' +
        ...
        if (payload.invoiceId) {
            ...
            if (!invoice) {
                throw new Error(`ACCOUNTING_VIOLATION: Invoice not found: ${payload.invoiceId}. ` +
            ...
            if (!isAdvance && invoice.status === 'paid') {
                throw new Error(`ACCOUNTING_VIOLATION: Invoice is already fully paid. ` +
            ...
            if (!isAdvance && invoice.remainingAmount < payload.amount) {
                throw new Error(`ACCOUNTING_VIOLATION: Payment amount exceeds outstanding invoice amount. ` +
        ...
        if (payload.referenceNumber && ['bank', 'online_transfer', 'card'].includes(payload.paymentMode)) {
            const existingPayment = await client_1.default.payment.findFirst({
                where: {
            ...
            if (existingPayment) {
                throw new Error(`ACCOUNTING_VIOLATION: Duplicate reference number. ` +
    ...
    static async validateTransactionCreation(payload) {
        // BLOCK: Amount ≤ 0
        if (!payload.amount || payload.amount <= 0) {
            throw new Error('ACCOUNTING_VIOLATION: Transaction amount must be greater than zero. ' +
        ...
        if ((payload.debitAccountId || payload.creditAccountId) &&
            ...
            !hasTenantContext &&
            !hasDealerContext) {
            throw new Error('ACCOUNTING_VIOLATION: Transaction must resolve to invoice creation, payment recording, or advance handling. ' +
        ...
        if (payload.transactionType === 'income' && !hasInvoiceContext) {
            const creditAccount = payload.creditAccountId
                ? await client_1.default.account.findUnique({ where: { id: payload.creditAccountId }, select: { type: true } })
                : null;
            if (creditAccount?.type === 'Revenue' && !hasInvoiceContext) {
                throw new Error('ACCOUNTING_VIOLATION: Revenue cannot be created without invoice. ' +
        ...
        if (payload.debitAccountId || payload.creditAccountId) {
            const debitAccount = payload.debitAccountId
                ? await client_1.default.account.findUnique({ where: { id: payload.debitAccountId }, select: { code: true, name: true } })
                : null;
            const creditAccount = payload.creditAccountId
                ? await client_1.default.account.findUnique({ where: { id: payload.creditAccountId }, select: { code: true, name: true } })
                : null;
            const isCashBankDebit = debitAccount && (debitAccount.code?.startsWith('1111') || debitAccount.code?.startsWith('1112') ||
                debitAccount.name?.toLowerCase().includes('cash') || debitAccount.name?.toLowerCase().includes('bank'));
            const isCashBankCredit = creditAccount && (creditAccount.code?.startsWith('1111') || creditAccount.code?.startsWith('1112') ||
            ...
            if ((isCashBankDebit || isCashBankCredit) && !hasInvoiceContext && !hasTenantContext && !hasDealerContext) {
                throw new Error('ACCOUNTING_VIOLATION: Cash/Bank movement requires payment context. ' +
     ...
     */
    static async validateAccountTypeForTransaction(accountId, side, transactionType) {
        const account = await client_1.default.account.findUnique({
            where: { id: accountId },
            select: { type: true, code: true, name: true },
        });
    ...
    static async validateDealPayment(payload) {
        // BLOCK: Deal is not selected
        if (!payload.dealId) {
            throw new Error('ACCOUNTING_VIOLATION: Deal is required for deal payment. ' +
                'Deal payments must be linked to a deal.');
        }
        // BLOCK: Amount ≤ 0
        if (!payload.amount || payload.amount <= 0) {
            throw new Error('ACCOUNTING_VIOLATION: Deal payment amount must be greater than zero. ' +
                `Received: ${payload.amount}. Zero-value payments are not allowed.`);
        }
        ...
        if (deal.status === 'Closed' || deal.status === 'Cancelled') {
            throw new Error(`ACCOUNTING_VIOLATION: Cannot record payment for closed/cancelled deal. ` +
     ...
     */
    static validateDoubleEntryBalance(lines) {
        const totalDebit = lines.reduce((sum, line) => sum + (line.debit || 0), 0);
        const totalCredit = lines.reduce((sum, line) => sum + (line.credit || 0), 0);
        const difference = Math.abs(totalDebit - totalCredit);
        if (difference > 0.01) { // Allow for floating point precision
            throw new Error(`ACCOUNTING_VIOLATION: Double-entry imbalance detected. ` +
     ...
     */
    static async validateRecordNotPosted(recordType, recordId) {
        let record = null;
        switch (recordType) {
            case 'invoice':
                record = await client_1.default.invoice.findUnique({
                    where: { id: recordId },
                    select: { journalEntryId: true, status: true },
                });
                if (record?.journalEntryId) {
                    throw new Error(`ACCOUNTING_VIOLATION: Invoice has been posted (journal entry created). ` +
                        `Posted invoices cannot be edited. Use reversal logic instead.`);
                }
                break;
            case 'payment':
                record = await client_1.default.payment.findUnique({
                    where: { id: recordId },
                    select: { id: true },
                });
                ...
                if (ledgerEntries.length > 0) {
                    throw new Error(`ACCOUNTING_VIOLATION: Payment has been posted (ledger entries created). ` +
                        `Posted payments cannot be edited. Use reversal logic instead.`);
                }
                break;
            case 'transaction':
                record = await client_1.default.transaction.findUnique({
                    where: { id: recordId },
                    select: { journalEntryId: true },
                });
                if (record?.journalEntryId) {
                    throw new Error(`ACCOUNTING_VIOLATION: Transaction has been posted (journal entry created). ` +
     ...
     */
    static async validateDuplicateInvoiceNumber(invoiceNumber, excludeId) {
        const existing = await client_1.default.invoice.findFirst({
            where: {
                invoiceNumber,
        ...
        if (existing) {
            throw new Error(`ACCOUNTING_VIOLATION: Duplicate invoice number. ` +
     ...
     */
    static async validateDuplicateReference(referenceType, referenceNumber, context) {
        ...
        if (referenceType === 'cheque' && context?.paymentMode === 'bank') {
            ...
            if (existingPayment) {
                throw new Error(`ACCOUNTING_VIOLATION: Duplicate cheque number. ` +
...
exports.AccountingSafetyService = AccountingSafetyService;
//# sourceMappingURL=accounting-safety-service.js.map