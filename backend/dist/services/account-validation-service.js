...
class AccountValidationService {
     ...
     */
    static async validateAccountPostable(accountId) {
        ...
        if (account.accountType === 'Header') {
            throw new Error(`Cannot post to Header account: ${account.code} - ${account.name} (Level ${account.level}). ` +
        ...
        if (!account.isPostable) {
            throw new Error(`Cannot post to account: ${account.code} - ${account.name}. ` +
        ...
        if (account.level !== 5 || account.accountType !== 'Posting') {
            throw new Error(`Account ${account.code} - ${account.name} is not a Level-5 Posting account. ` +
     ...
     * - Refund to clients
     */
    static async validateTrustAccountUsage(accountId, transactionType, counterpartAccountId) {
        const account = await client_1.default.account.findUnique({
            where: { id: accountId },
        });
        if (!account) {
        ...
        if (counterpartAccountId) {
            const counterpartAccount = await client_1.default.account.findUnique({
            ...
            if (counterpartAccount.type === 'Expense') {
                throw new Error(`❌ Trust accounts cannot pay expenses. ` +
            ...
            if (counterpartAccount.type === 'Revenue') {
                throw new Error(`❌ Trust accounts cannot receive revenue. ` +
            ...
            if (operatingCashCodes.includes(counterpartAccount.code)) {
                ...
                console.warn(`Trust account ${account.code} used with operating account ${counterpartAccount.code}. ` +
     ...
     */
    static async validateRevenuePosting(debitAccountId, creditAccountId) {
        const [debitAccount, creditAccount] = await Promise.all([
            client_1.default.account.findUnique({ where: { id: debitAccountId } }),
            client_1.default.account.findUnique({ where: { id: creditAccountId } }),
        ]);
        ...
        if (operatingCashBankCodes.includes(debitAccount.code)) {
            throw new Error(`❌ Revenue cannot post directly from cash/bank accounts. ` +
        ...
        if (debitAccount.trustFlag || debitAccount.code.startsWith('1121')) {
            ...
            console.warn(`Revenue posting from trust account: ${debitAccount.code} - ${debitAccount.name}. ` +
     ...
     */
    static async validateAdvancePosting(debitAccountId, creditAccountId) {
        const creditAccount = await client_1.default.account.findUnique({
        ...
        const debitAccount = await client_1.default.account.findUnique({
        ...
        if (debitAccount &&
            ...
            if (accountName.includes('advance') ||
                ...
                throw new Error(`Advances must be posted to liability accounts (2101, 2102, 2103), ` +
    ...
    static validateDoubleEntryBalance(lines) {
        ...
        if (Math.abs(totalDebits - totalCredits) > 0.01) {
            throw new Error(`Journal entry must balance. Total debits: ${totalDebits.toFixed(2)}, ` +
     ...
     */
    static calculateAccountBalance(account, debitTotal, creditTotal) {
    ...
    static async getPostableAccounts(filters) {
        ...
        return await client_1.default.account.findMany({
            where,
            orderBy: [{ code: 'asc' }],
            include: {
                parent: {
     ...
     */
    static async getAccountsForDropdown(dropdownType) {
        switch (dropdownType) {
            case 'journal':
            case 'voucher':
            case 'transaction':
            ...
            case 'invoice-tenant':
                // Only AR accounts
                return await this.getPostableAccounts({ type: 'Asset' }).then((accounts) => accounts.filter((acc) => acc.code.startsWith('11')));
            case 'invoice-income':
                // Only Revenue accounts
                return await this.getPostableAccounts({ type: 'Revenue' });
            case 'payment-debit':
                // Only Cash/Bank accounts
                return await this.getPostableAccounts({ type: 'Asset' }).then((accounts) => accounts.filter((acc) => ['1001', '1002', '1003', '1011', '1014'].includes(acc.code)));
            case 'payment-credit':
                // Only AR accounts
                return await this.getPostableAccounts({ type: 'Asset' }).then((accounts) => accounts.filter((acc) => acc.code.startsWith('11')));
            default:
     ...
     */
    static async validateEscrowBalance() {
        ...
        const trustLiabilityAccounts = await client_1.default.account.findMany({
            where: {
                OR: [
     ...
     */
    static async validatePropertyUnitRequirement(accountId, propertyId, unitId) {
        const account = await client_1.default.account.findUnique({
            where: { id: accountId },
        });
        if (!account) {
        ...
        const requiresProperty = account.type === 'Revenue' ||
            (account.type === 'Expense' &&
                !expenseWithoutPropertyAllowedCodes.includes(account.code));
        if (requiresProperty) {
            if (!propertyId) {
                throw new Error(`Property ID is required for ${account.type} accounts. ` +
            ...
            if (account.type === 'Revenue' &&
                ...
                if (!unitId) {
                    throw new Error(`Unit ID is required for ${account.name}. ` +
                        `Account: ${account.code} - ${account.name}. ` +
     ...
     */
    static async validateJournalEntry(lines) {
        // Validate double-entry balance
        ...
        for (const line of lines) {
            ...
            await this.validatePropertyUnitRequirement(line.accountId, line.propertyId, line.unitId);
            // Find counterpart account (if any)
            const counterpartLine = lines.find((l) => l.accountId !== line.accountId && (l.debit > 0 || l.credit > 0));
            if (counterpartLine) {
                // Validate trust account usage
                if (line.debit > 0) {
                    await this.validateTrustAccountUsage(line.accountId, 'debit', counterpartLine.accountId);
                }
                if (line.credit > 0) {
                    await this.validateTrustAccountUsage(line.accountId, 'credit', counterpartLine.accountId);
                }
                ...
                if (account && counterpartAccount) {
                    if (account.type === 'Revenue' || counterpartAccount.type === 'Revenue') {
                        await this.validateRevenuePosting(line.debit > 0 ? line.accountId : counterpartLine.accountId, line.credit > 0 ? line.accountId : counterpartLine.accountId);
                    }
                }
            }
        }
