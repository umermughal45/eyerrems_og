...
class ReceiptService {
    ...
    static async generateReceiptNumber() {
        return await (0, id_generation_service_1.generateSystemId)('rcp');
    }
    /**
    ...
    static async createReceipt(payload) {
        ...
        if (payload.manualUniqueId) {
            await (0, id_generation_service_1.validateManualUniqueId)(payload.manualUniqueId, 'rcp');
        }
        ...
        const trustCashAccount = await client_1.default.account.findFirst({
            where: {
                AND: [{ isActive: true }],
                OR: [
        ...
        const trustBankAccount = await client_1.default.account.findFirst({
            where: {
                AND: [{ isActive: true }],
                OR: [
        ...
        const installmentReceivableAccount = await client_1.default.account.findFirst({
            where: {
                OR: [
                    { code: '1100' },
                    { code: '201' },
        ...
        const advanceLiabilityAccount = await client_1.default.account.findFirst({
            where: {
        ...
        if (!cashAccount || !bankAccount || (!installmentReceivableAccount && !advanceLiabilityAccount)) {
            ...
            if (!bankAccount)
                missing.push('Bank');
            if (!installmentReceivableAccount && !advanceLiabilityAccount)
                missing.push('Accounts Receivable or Client Advances Payable');
        ...
        return await client_1.default.$transaction(async (tx) => {
            ...
            const allocations = await this.allocateReceiptFIFO(receipt.id, payload.dealId, payload.amount, tx);
            ...
            await account_validation_service_1.AccountValidationService.validateJournalEntry(journalLines.map((l) => ({
            ...
            if (deal && !payload.skipPaymentCreation) {
                ...
                if (!existingPayment) {
                    ...
                    const paymentCode = await (0, id_generation_service_1.generateSystemId)('pay');
                    ...
                    const { debitAccountId, creditAccountId } = await PaymentService.getPaymentAccounts(payload.method.toLowerCase());
            ...
            if (deal) {
                ...
                const installmentPaidAmount = deal.installments.reduce((sum, inst) => sum + (inst.paidAmount || 0), 0);
                ...
                const totalAllocated = allocations.reduce((sum, alloc) => sum + alloc.amountAllocated, 0);
                ...
                const isDownPaymentPaid = downPayment > 0 && ((payload.amount === downPayment && totalAllocated === 0) || // Payment exactly for down payment
                ...
                if (deal.paymentPlan) {
                    ...
                    if (paymentPlan) {
                        const totalExpected = paymentPlan.installments.reduce((sum, inst) => sum + inst.amount, 0);
                        const installmentPaidAmount = paymentPlan.installments.reduce((sum, inst) => sum + (inst.paidAmount || 0), 0);
                        // Only include down payment if it's been paid (unallocated amount covers it or payment exactly matches)
                        const isDownPaymentPaidForPlan = downPayment > 0 && ((payload.amount === downPayment && totalAllocated === 0) || // Payment exactly for down payment
                            unallocatedAmount >= downPayment // Unallocated amount covers down payment
                        );
     ...
     */
    static async allocateReceiptFIFO(receiptId, dealId, receiptAmount, tx) {
        const allocations = [];
     ...
     */
    static async getReceiptsByDealId(dealId) {
        return await client_1.default.dealReceipt.findMany({
            where: {
                dealId,
            },
            include: {
    ...
    static async getReceiptById(receiptId) {
        return await client_1.default.dealReceipt.findUnique({
            where: { id: receiptId },
            include: {
                deal: {
                    include: {
