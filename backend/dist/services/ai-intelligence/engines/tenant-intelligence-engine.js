...
exports.TenantIntelligenceEngine = void 0;
const client_1 = __importDefault(require("../../../prisma/client"));
const types_1 = require("../types");
const logger_1 = __importDefault(require("../../../utils/logger"));
const probability_models_1 = require("../ml/probability-models");
class TenantIntelligenceEngine {
    constructor() {
        this.name = 'TenantIntelligenceEngine';
        this.config = {
            data_sources: [
                {
                    module: 'Properties',
    ...
    async compute() {
        ...
        try {
            if (!(await this.hasSufficientData())) {
                return {
                    insights: [(0, types_1.createInsufficientDataInsight)('Tenant Intelligence', this.getDataSources())],
            ...
            tenants.forEach((tenant) => {
                ...
                const expiringLeases = tenantLeases.filter((l) => {
            ...
            const onTimePayments = payments.filter((p) => {
                if (p.status !== 'paid')
                    return false;
                const invoice = invoices.find((i) => i.tenantId === p.tenantId);
                if (!invoice)
                    return true;
            ...
            const tenantChurnProbabilities = [];
            tenants.forEach((tenant) => {
                const tenantPayments = payments.filter((p) => p.tenantId === tenant.id);
                const tenantLeases = leases.filter((l) => l.tenantId === tenant.id);
                if (tenantPayments.length >= 3) {
                    ...
                    const paymentDelays = tenantPayments.filter((p) => {
                        if (p.status === 'paid')
                            return false;
                        const daysSince = (new Date().getTime() - new Date(p.date).getTime()) / (1000 * 60 * 60 * 24);
                        return daysSince > 15;
                    ...
                    const churnResult = (0, probability_models_1.calculateChurnProbability)({
                        paymentDelays,
            ...
            if (tenantChurnProbabilities.length > 0) {
                ...
                const leasesExpiringIn3Months = leases.filter((l) => {
                    const endDate = new Date(l.leaseEnd);
                    const daysUntilExpiry = (endDate.getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24);
                    return daysUntilExpiry > 0 && daysUntilExpiry <= 90;
                });
            ...
            const churnConfidence = tenantChurnProbabilities.length > 0
                ...
                : (0, types_1.calculateConfidence)(70, confidenceFactors);
            ...
            logger_1.default.error(`Tenant Intelligence Engine error: ${error.message}`, error);
            errors.push(error.message);
            insights.push((0, types_1.createErrorInsight)('Tenant Intelligence', error.message, this.getDataSources()));
        }
        return {
            insights,
            engine_name: this.name,
    ...
    async hasSufficientData() {
        try {
            const tenantCount = await client_1.default.tenant.count({
                where: { isDeleted: false },
            });
            return tenantCount > 0;
        }
        catch (error) {
            logger_1.default.error('Error checking sufficient data for Tenant Intelligence', error);
            return false;
        }
    }
    getDataSources() {
...
exports.TenantIntelligenceEngine = TenantIntelligenceEngine;
//# sourceMappingURL=tenant-intelligence-engine.js.map