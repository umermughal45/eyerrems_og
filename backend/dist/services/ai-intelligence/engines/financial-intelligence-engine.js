...
exports.FinancialIntelligenceEngine = void 0;
const client_1 = __importDefault(require("../../../prisma/client"));
const types_1 = require("../types");
const logger_1 = __importDefault(require("../../../utils/logger"));
const time_series_forecast_1 = require("../ml/time-series-forecast");
const correctness_contracts_1 = require("../validation/correctness-contracts");
const contracts_1 = require("../validation/contracts");
const data_legitimacy_1 = require("../validation/data-legitimacy");
const decision_logger_1 = require("../audit/decision-logger");
class FinancialIntelligenceEngine {
    constructor() {
        this.name = 'FinancialIntelligenceEngine';
        this.config = {
            data_sources: [
                {
                    module: 'Finance',
    ...
    async compute() {
        ...
        try {
            ...
            if (!(await this.hasSufficientData())) {
                return {
                    insights: [
                        (0, types_1.createInsufficientDataInsight)('Financial Intelligence', this.getDataSources()),
                    ],
            ...
            const allTransactions = await client_1.default.transaction.findMany({
                where: {
            ...
            const transactions = allTransactions.filter((t) => !data_legitimacy_1.FinanceLegitimacy.excludeReversedTransactions(t));
            // Get invoices (will be filtered for legitimacy)
            const allInvoices = await client_1.default.invoice.findMany({
                where: {
            ...
            const invoices = allInvoices.filter((i) => !data_legitimacy_1.FinanceLegitimacy.excludeDraftInvoices(i));
            // Get tenant payments (not deal payments) - will be filtered for legitimacy
            const allPayments = await client_1.default.tenantPayment.findMany({
                where: {
                    date: { gte: twelveMonthsAgo },
                },
                select: {
            ...
            const payments = allPayments.filter((p) => p.status !== 'draft');
            ...
            const revenueTransactions = transactions.filter((t) => t.transactionType === 'income' || t.transactionType === 'credit');
            ...
            const hasManualOverrides = transactions.some((t) => {
            ...
            const hasBackdatedEntries = transactions.some((t) => {
                const created = new Date(t.createdAt);
                const tDate = new Date(t.date);
            ...
            const lastMonthRevenueTransactions = transactions.filter((t) => {
                const tDate = new Date(t.date);
            ...
            const contractResult = (0, correctness_contracts_1.validateContract)(revenueTransactions, contracts_1.RevenuePredictionContract, { start: twelveMonthsAgo, end: new Date() });
            ...
            if (!contractResult.passed) {
                ...
                const refusalInsight = (0, correctness_contracts_1.createRefusalInsight)('Revenue Prediction', contractResult.failureReason || 'Contract validation failed', this.getDataSources());
                (0, decision_logger_1.createDecisionLog)(this.name, { type: 'refusal', refusalReason: contractResult.failureReason }, {
                ...
                const anomalyPct = (0, data_legitimacy_1.calculateAnomalyPercentage)(monthlyRevenues);
                ...
                const timeConsistency = (0, data_legitimacy_1.isTimeRangeConsistent)(legitimateRevenueTransactions, 'date', 90);
                // ML-based revenue forecasting (APPROVED USE CASE)
                // Rule-based fallback if ML confidence insufficient
                const mlForecast = (0, time_series_forecast_1.forecastTimeSeries)(monthlyRevenues, 1, 6, 70);
                ...
                if (mlForecast.method === 'ml' && mlForecast.prediction !== null) {
                    ...
                    const recentTrend = monthlyRevenues.slice(-3).reduce((a, b) => a + b, 0) / 3;
                    predictedRevenue = avgRevenue * 0.6 + recentTrend * 0.4;
                    predictionMethod = 'Rule-based (moving average)';
                    predictionConfidence = 75;
                }
                const actualRevenueConfidence = (0, types_1.calculateConfidence)(95, confidenceFactors);
                ...
                if (finalConfidence < 60) {
                    const refusalInsight = (0, correctness_contracts_1.createRefusalInsight)('Revenue Prediction', `Confidence ${finalConfidence.toFixed(1)}% below minimum threshold (60%). Insufficient data quality for reliable prediction.`, this.getDataSources());
                    (0, decision_logger_1.createDecisionLog)(this.name, { type: 'refusal', refusalReason: `Low confidence: ${finalConfidence.toFixed(1)}%` }, {
                        totalRecords: contractResult.context.totalRecords,
                        legitimateRecords: contractResult.legitimateCount,
                        excludedRecords: contractResult.excludedCount,
                        timeRange: { start: twelveMonthsAgo, end: new Date() },
                    ...
                    // Log decision
                    (0, decision_logger_1.createDecisionLog)(this.name, { type: 'insight', insight }, {
                        totalRecords: contractResult.context.totalRecords,
                        legitimateRecords: contractResult.legitimateCount,
                        excludedRecords: contractResult.excludedCount,
                        timeRange: { start: twelveMonthsAgo, end: new Date() },
            ...
            const lastMonthExpenseTransactions = transactions.filter((t) => {
                const tDate = new Date(t.date);
                return tDate >= lastMonthStart && tDate <= lastMonthEnd &&
                    (t.transactionType === 'expense' || t.transactionType === 'debit');
            });
            const profitConfidence = (0, types_1.calculateConfidence)(90, confidenceFactors);
            ...
            const paymentRiskResult = (0, correctness_contracts_1.validateContract)(payments, contracts_1.PaymentRiskContract, { start: twelveMonthsAgo, end: new Date() });
            if (!paymentRiskResult.passed) {
                const refusalInsight = (0, correctness_contracts_1.createRefusalInsight)('Payment Risk Assessment', paymentRiskResult.failureReason || 'Contract validation failed', this.getDataSources());
                insights.push(refusalInsight);
            }
            else {
                const legitimatePayments = payments.slice(0, paymentRiskResult.legitimateCount);
                const overduePayments = legitimatePayments.filter((p) => {
                    const paymentDate = new Date(p.date);
                    const daysSince = (new Date().getTime() - paymentDate.getTime()) / (1000 * 60 * 60 * 24);
                    return p.status !== 'paid' && daysSince > 15;
                });
                const paymentRiskConfidence = (0, types_1.calculateConfidence)(88, confidenceFactors);
            ...
            logger_1.default.error(`Financial Intelligence Engine error: ${error.message}`, error);
            errors.push(error.message);
            insights.push((0, types_1.createErrorInsight)('Financial Intelligence', error.message, this.getDataSources()));
        }
        return {
            insights,
            engine_name: this.name,
    ...
    async hasSufficientData() {
        try {
            ...
            logger_1.default.error('Error checking sufficient data for Financial Intelligence', error);
            return false;
        }
    }
    getDataSources() {
    ...
    getConfidenceReason(factors, confidence) {
        const reasons = [];
...
exports.FinancialIntelligenceEngine = FinancialIntelligenceEngine;
//# sourceMappingURL=financial-intelligence-engine.js.map