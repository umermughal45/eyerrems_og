...
exports.PayrollAccountingService = void 0;
...
class PayrollAccountingService {
     ...
     */
    static async postPayrollApproval(context) {
        ...
        if (!mappings) {
            throw new Error('PAYROLL_ACCOUNTING_ERROR: Payroll account mappings not configured. ' +
                'Please configure Salary Expense and Salary Payable accounts in system settings.');
        }
        // Check if already posted
        const existingPayroll = await client_1.default.payroll.findUnique({
            where: { id: payrollId },
        ...
        const [expenseAccount, payableAccount] = await Promise.all([
            client_1.default.account.findUnique({ where: { id: mappings.salaryExpenseAccountId } }),
            client_1.default.account.findUnique({ where: { id: mappings.salaryPayableAccountId } }),
        ]);
        if (!expenseAccount || !expenseAccount.isActive) {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Salary Expense account not found or inactive: ${mappings.salaryExpenseAccountId}`);
        }
        if (!payableAccount || !payableAccount.isActive) {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Salary Payable account not found or inactive: ${mappings.salaryPayableAccountId}`);
        }
        if (expenseAccount.type !== 'Expense') {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Account ${expenseAccount.name} is not an Expense account. Salary Expense must be an Expense type.`);
        }
        if (payableAccount.type !== 'Liability') {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Account ${payableAccount.name} is not a Liability account. Salary Payable must be a Liability type.`);
        }
        ...
        return await client_1.default.$transaction(async (tx) => {
            ...
            const entryNumber = await (0, id_generation_service_1.generateSystemId)('je'); // Journal Entry
     ...
     */
    static async postPayrollPayment(context) {
        ...
        if (!payroll.journalEntryId) {
            throw new Error('PAYROLL_ACCOUNTING_ERROR: Payroll must be approved and posted before payment can be recorded. ' +
        ...
        if (amount > remainingBalance) {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Payment amount (${amount}) exceeds remaining Salary Payable balance (${remainingBalance}). ` +
        ...
        else if (['bank', 'transfer', 'cheque', 'bank_transfer', 'online'].includes(normalizedMethod) &&
        ...
        if (!paymentAccountId) {
            throw new Error('PAYROLL_ACCOUNTING_ERROR: Cash or Bank account not configured. ' +
                'Please configure payment accounts in system settings.');
        }
        // Validate payment account
        const paymentAccount = await client_1.default.account.findUnique({
            where: { id: paymentAccountId },
        });
        if (!paymentAccount || !paymentAccount.isActive) {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Payment account not found or inactive: ${paymentAccountId}`);
        }
        if (paymentAccount.type !== 'Asset') {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Account ${paymentAccount.name} is not an Asset account. ` +
                `Payment account must be Cash or Bank (Asset type).`);
        }
        // Validate salary payable account
        const payableAccount = await client_1.default.account.findUnique({
        ...
        const payment = await client_1.default.payrollPayment.findUnique({
            where: { id: paymentId },
            select: { id: true },
        });
        if (!payment) {
        ...
        const existingJournalEntry = await client_1.default.journalEntry.findFirst({
            where: {
                voucherNo: voucherNoPattern,
            },
        });
        if (existingJournalEntry) {
            throw new Error(`PAYROLL_ACCOUNTING_ERROR: Payment has already been posted to ledger. ` +
        ...
        return await client_1.default.$transaction(async (tx) => {
            ...
            if (Math.abs(totalDebit - totalCredit) > 0.01) {
                throw new Error(`PAYROLL_ACCOUNTING_ERROR: Double-entry balance mismatch. Debit: ${totalDebit}, Credit: ${totalCredit}`);
            }
            // Generate entry number
            const entryNumber = await (0, id_generation_service_1.generateSystemId)('je'); // Journal Entry
            ...
            if (!journalEntry.lines || journalEntry.lines.length !== 2) {
                throw new Error(`PAYROLL_ACCOUNTING_ERROR: Journal entry created with incorrect number of lines. Expected 2, got ${journalEntry.lines?.length || 0}`);
            }
            ...
            if (Math.abs(createdDebit - createdCredit) > 0.01 || Math.abs(createdDebit - amount) > 0.01) {
                throw new Error(`PAYROLL_ACCOUNTING_ERROR: Journal entry balance verification failed. Debit: ${createdDebit}, Credit: ${createdCredit}, Expected: ${amount}`);
            }
     ...
     */
    static async validatePayrollPosting() {
        const mappings = await this.getAccountMappings();
        if (!mappings) {
            return {
                valid: false,
     ...
     */
    static async validatePaymentPosting(payrollId, amount) {
        const mappings = await this.getAccountMappings();
        if (!mappings) {
            return {
                valid: false,
...
exports.PayrollAccountingService = PayrollAccountingService;
//# sourceMappingURL=payroll-accounting-service.js.map