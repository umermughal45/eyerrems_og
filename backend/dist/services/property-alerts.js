...
exports.getMaintenanceDueAlerts = getMaintenanceDueAlerts;
exports.getLeaseExpiryAlerts = getLeaseExpiryAlerts;
exports.getAllPropertyAlerts = getAllPropertyAlerts;
const client_1 = __importDefault(require("../prisma/client"));
/**
 * Check for maintenance requests that are due
 * Returns maintenance requests that need attention
 */
async function getMaintenanceDueAlerts(propertyId) {
    const where = {
        isDeleted: false,
    ...
    const urgentMaintenance = await client_1.default.maintenanceRequest.findMany({
        where: {
            ...where,
    ...
    const overdueMaintenance = await client_1.default.maintenanceRequest.findMany({
        where: {
            ...where,
            status: 'in-progress',
 ...
 */
async function getLeaseExpiryAlerts(propertyId) {
    ...
    const leasesExpiringIn30Days = expiringLeases.filter((lease) => {
        const daysRemaining = Math.ceil((lease.leaseEnd.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        return daysRemaining <= 30 && daysRemaining > 15;
    });
    const leasesExpiringIn15Days = expiringLeases.filter((lease) => {
        const daysRemaining = Math.ceil((lease.leaseEnd.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        return daysRemaining <= 15 && daysRemaining > 7;
    });
    const leasesExpiringIn7Days = expiringLeases.filter((lease) => {
        const daysRemaining = Math.ceil((lease.leaseEnd.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        return daysRemaining <= 7 && daysRemaining > 0;
    });
    const expiredLeases = expiringLeases.filter((lease) => lease.leaseEnd < today);
    return {
 ...
 */
async function getAllPropertyAlerts(propertyId) {
...
//# sourceMappingURL=property-alerts.js.map