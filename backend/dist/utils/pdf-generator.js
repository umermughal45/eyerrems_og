...
exports.generatePaymentPlanPDF = generatePaymentPlanPDF;
exports.generateReceiptPDF = generateReceiptPDF;
exports.generatePropertyReportPDF = generatePropertyReportPDF;
exports.generatePropertiesReportPDF = generatePropertiesReportPDF;
 ...
 */
function generatePaymentPlanPDF(data, res) {
    try {
        // #region agent log
        ...
        res.setHeader('Content-Disposition', `attachment; filename="payment-plan-${data.deal.dealCode || 'report'}-${new Date().toISOString().split('T')[0]}.pdf"`);
        // #region agent log
        ...
        const footerAddedToPages = new Set();
        ...
        const addFooter = () => {
            ...
            const currentPage = doc.page.pageNumber || 0;
            ...
            try {
                ...
                const writableState = doc._writableState;
                const readableState = doc._readableState;
                ...
                doc.text('This is a computer-generated document. No signature required. | Real Estate Management System', pageWidth / 2, footerY, { align: 'center', width: pageWidth - 100 });
                // Restore position
                doc.x = savedX;
                doc.y = savedY;
            }
        ...
        if (data.deal.property) {
            ...
            if (data.deal.property.propertyCode)
                doc.text(`Property Code: ${data.deal.property.propertyCode}`);
        }
        if (data.deal.dealer) {
        ...
        if (data.deal.client) {
            ...
            if (data.deal.client.phone)
                doc.text(`Phone: ${data.deal.client.phone}`);
            if (data.deal.client.email)
                doc.text(`Email: ${data.deal.client.email}`);
        }
        doc.moveDown(0.6);
        // ============ PAYMENT SUMMARY ============
        ...
        const lastPage = doc.page.pageNumber || 0;
...
async function generateReceiptPDF(data) {
    return new Promise((resolve, reject) => {
        ...
        if (data.client.address)
            doc.text(`Address: ${data.client.address}`);
        doc.moveDown(1);
        // Deal Information
        doc.fontSize(14).font('Helvetica-Bold').text('Deal Information', { underline: true });
        doc.moveDown(0.5);
        ...
        // Footer note
        doc.fontSize(8).font('Helvetica');
        doc.text('This is a computer-generated receipt. No signature required.', pageWidth / 2, pageHeight - 50, { align: 'center' });
        doc.end();
    });
}
/**
 ...
 */
function generatePropertyReportPDF(data, res) {
    const doc = new pdfkit_1.default({ margin: 50, size: 'A4' });
    res.setHeader('Content-Type', 'application/pdf');
    const safeName = data.property.name?.replace(/\s+/g, '-').toLowerCase() || 'property';
    res.setHeader('Content-Disposition', `attachment; filename="${safeName}-report-${new Date().toISOString().split('T')[0]}.pdf"`);
    doc.pipe(res);
    const formatCurrency = (amount) => {
    ...
    if (data.property.yearBuilt)
        doc.text(`Year Built: ${data.property.yearBuilt}`);
    if (data.property.totalArea)
        doc.text(`Total Area: ${data.property.totalArea.toLocaleString()} sq ft`);
    doc.moveDown(0.6);
    // Pricing & ownership
    ...
    doc.text(`Units: ${data.property.occupied || 0} occupied of ${data.property.totalUnits ?? 0}`);
    doc.moveDown(0.6);
    // Finance summary
    ...
    if (!data.deals || data.deals.length === 0) {
        ...
        data.deals.forEach((deal) => {
            ...
            doc.text(`  Client: ${deal.clientName || 'N/A'} | Dealer: ${deal.dealerName || 'N/A'} | Stage: ${deal.stage || 'N/A'} | Created: ${formatDate(deal.createdAt)}`);
            doc.moveDown(0.2);
        });
    }
    doc.moveDown(0.6);
    ...
    if (!data.sales || data.sales.length === 0) {
        ...
        data.sales.forEach((sale) => {
            doc.text(`Sale: ${formatCurrency(sale.saleValue)} | Buyer: ${sale.buyerName || 'N/A'} | Dealer: ${sale.dealerName || 'N/A'} | Status: ${sale.status || 'N/A'} | Date: ${formatDate(sale.saleDate)}`);
    ...
    const pageWidth = doc.page.width;
    doc.fontSize(8).font('Helvetica');
    doc.text('Generated by REMS - Property Report', pageWidth / 2, pageHeight - 40, { align: 'center' });
    doc.end();
}
/**
 * Generate Properties PDF Report - audit-grade grid list
 */
async function generatePropertiesReportPDF(data, res) {
    const { generateListReportPDF } = await Promise.resolve().then(() => __importStar(require('./audit-grade-pdf-report')));
    ...
    const columns = [
        { key: 'propertyCode', header: 'Property Code', width: 120, type: 'string' },
        { key: 'name', header: 'Name', width: 180, type: 'string' },
        { key: 'type', header: 'Type', width: 100, type: 'string' },
        { key: 'address', header: 'Address', width: 220, type: 'string' },
        {
            key: 'salePrice',
            header: 'Sale Price',
            width: 110,
            type: 'currency',
            format: (v) => v != null && !isNaN(Number(v))
                ? `Rs ${Number(v).toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
                : '—',
        },
        { key: 'subsidiary', header: 'Subsidiary', width: 120, type: 'string' },
    ];
    ...
    return new Promise((resolve, reject) => {
        ...
        if ((data.voucher.type === 'BPV' || data.voucher.type === 'CPV') && data.voucher.payeeType) {
            ...
            if (data.voucher.deal?.client) {
                ...
                if (data.voucher.deal.client.email)
                    doc.text(`Email: ${data.voucher.deal.client.email}`);
                if (data.voucher.deal.client.phone)
                    doc.text(`Phone: ${data.voucher.deal.client.phone}`);
            }
            doc.moveDown(0.5);
        }
        ...
        if (data.voucher.property || data.voucher.unit) {
            ...
            if (data.voucher.property) {
                ...
                if (data.voucher.property.code)
                    doc.text(`Property Code: ${data.voucher.property.code}`);
            }
            if (data.voucher.unit) {
                doc.text(`Unit: ${data.voucher.unit.unitName || '-'}`);
                if (data.voucher.unit.unitNumber)
                    doc.text(`Unit Number: ${data.voucher.unit.unitNumber}`);
            }
            doc.moveDown(0.5);
        }
        // ============ DEAL INFORMATION ============
        if (data.voucher.deal) {
            ...
            doc.moveDown(0.4);
            doc.fontSize(10).font('Helvetica');
            if (data.voucher.deal.dealCode)
                doc.text(`Deal Code: ${data.voucher.deal.dealCode}`);
        ...
        doc.moveDown(2);
        doc.fontSize(8).font('Helvetica');
        doc.text('This is a computer-generated voucher document. No signature required. | Real Estate Management System', pageWidth / 2, pageHeight - 40, { align: 'center', width: pageWidth - 100 });
        doc.end();
    });
}
