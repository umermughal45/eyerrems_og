...
function generateFinancialReportPDF(reportData, reportType, res) {
    ...
    // Footer
    doc.fontSize(8).font('Helvetica').text('This is a computer-generated report. For official use only.', { align: 'center' });
    doc.end();
}
...
function generateTrialBalancePDF(doc, data, formatCurrency, formatDate) {
    if (data.period?.startDate || data.period?.endDate) {
        doc.fontSize(10).font('Helvetica');
        if (data.period.startDate)
            doc.text(`From: ${formatDate(data.period.startDate)}`);
        if (data.period.endDate)
            doc.text(`To: ${formatDate(data.period.endDate)}`);
        doc.moveDown(0.5);
    }
    // Table header
...
function generateBalanceSheetPDF(doc, data, formatCurrency, formatDate) {
    if (data.asOfDate) {
        doc.fontSize(10).font('Helvetica');
    ...
    data.assets.current.forEach((entry) => {
    ...
    data.assets.fixed.forEach((entry) => {
        doc.text(`${entry.accountCode} - ${entry.accountName}`, 70, doc.y);
        doc.text(formatCurrency(entry.balance), 450, doc.y, { align: 'right' });
    ...
    data.liabilities.current.forEach((entry) => {
        doc.text(`${entry.accountCode} - ${entry.accountName}`, 70, doc.y);
    ...
    ['capital', 'retainedEarnings', 'currentYearProfit'].forEach((section) => {
        if (data.equity[section] && data.equity[section].length > 0) {
            doc.fontSize(9).font('Helvetica');
            data.equity[section].forEach((entry) => {
                doc.text(`${entry.accountCode} - ${entry.accountName}`, 70, doc.y);
                doc.text(formatCurrency(Math.abs(entry.balance)), 450, doc.y, { align: 'right' });
...
function generateProfitLossPDF(doc, data, formatCurrency, formatDate) {
    if (data.period?.startDate || data.period?.endDate) {
        doc.fontSize(10).font('Helvetica');
    ...
    ['propertyRevenue', 'serviceIncome'].forEach((section) => {
        if (data.revenue[section] && data.revenue[section].length > 0) {
            doc.fontSize(9).font('Helvetica');
            data.revenue[section].forEach((entry) => {
                doc.text(`${entry.accountCode} - ${entry.accountName}`, 70, doc.y);
                doc.text(formatCurrency(entry.balance), 450, doc.y, { align: 'right' });
    ...
    ['selling', 'property', 'administrative', 'tax'].forEach((section) => {
        if (data.expenses[section] && data.expenses[section].length > 0) {
            doc.fontSize(9).font('Helvetica');
            data.expenses[section].forEach((entry) => {
                doc.text(`${entry.accountCode} - ${entry.accountName}`, 70, doc.y);
                doc.text(formatCurrency(Math.abs(entry.balance)), 450, doc.y, { align: 'right' });
...
function generatePropertyProfitabilityPDF(doc, data, formatCurrency, formatDate) {
    ...
    profitabilityData.forEach((property, index) => {
        // Add page break for multiple properties (except first)
        if (index > 0) {
            doc.addPage();
        }
        // Property header
        doc.fontSize(14).font('Helvetica-Bold').text(property.propertyName || property.propertyId || 'Property Profitability', { align: 'center' });
        doc.moveDown(0.3);
        ...
        if (revenueBreakdown.length > 0) {
            revenueBreakdown.forEach((entry) => {
        ...
        if (expenseBreakdown.length > 0) {
            expenseBreakdown.forEach((entry) => {
                if (entry && entry.accountCode && entry.accountName) {
                    doc.fontSize(9).font('Helvetica');
...
function generateEscrowPDF(doc, data, formatCurrency, formatDate) {
    // Trust Assets
    ...
    doc.moveDown(0.3);
    doc.fontSize(9).font('Helvetica');
    data.clientLiabilities.forEach((liability) => {
...
function generateAgingPDF(doc, data, formatCurrency, formatDate) {
    doc.fontSize(10).font('Helvetica');
...
async function addFraudDetectionSection(doc, reportData, formatDate) {
    try {
        ...
        if (!startDate) {
            ...
            const redFlags = await fraud_detection_service_1.FraudDetectionService.generateRedFlagsReport(defaultStart, new Date());
            ...
            const redFlags = await fraud_detection_service_1.FraudDetectionService.generateRedFlagsReport(startDate instanceof Date ? startDate : new Date(startDate), endDate instanceof Date ? endDate : new Date(endDate));
            if (redFlags.length === 0)
                return;
            doc.addPage();
            doc.fontSize(12).font('Helvetica-Bold').text('FRAUD DETECTION & RED FLAGS', 50, doc.y);
        ...
        logger_1.default.warn('Failed to add fraud detection section:', error);
    }
}
//# sourceMappingURL=financial-report-pdf.js.map