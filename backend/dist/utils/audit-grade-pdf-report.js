...
exports.generateListReportPDF = generateListReportPDF;
exports.generateListReportPDFBuffer = generateListReportPDFBuffer;
exports.generateVoucherReportPDF = generateVoucherReportPDF;
 ...
 */
function generateListReportPDF(data, columns, options, res) {
    ...
    const doc = new pdfkit_1.default({ margin: PAGE_MARGIN, size: 'A4', layout: 'landscape' });
    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename="${reportTitle.replace(/\s+/g, '-')}-${generatedAt.toISOString().split('T')[0]}.pdf"`);
    doc.pipe(res);
    ...
    const addFooter = () => {
        ...
        doc.text(`Page ${doc.bufferedPageRange().count} | Generated ${generatedAt.toLocaleString('en-IN')}`, pageW / 2, doc.page.height - 24, { align: 'center', width: pageW - 80 });
    };
 ...
 */
function generateListReportPDFBuffer(data, columns, options) {
...
function stripDebugText(desc) {
    if (!desc)
        return '-';
    return desc.replace(/\s*\[SYSTEM\].*$/i, '').replace(/\s*\[.*?\]/g, '').trim() || '-';
}
function truncateDescription(s, maxLen = 70) {
 ...
 */
function generateVoucherReportPDF(data) {
    return new Promise((resolve, reject) => {
        ...
        doc.text('Voucher #:', sumLeft, y);
        doc.text(v.voucherNumber || '-', sumRight, y);
        y += 12;
        doc.text('Type:', sumLeft, y);
        doc.text(typeLabel, sumRight, y);
        y += 12;
        doc.text('Date:', sumLeft, y);
        doc.text(formatDate(v.date), sumRight, y);
        y += 12;
        doc.text('Total:', sumLeft, y);
        doc.font('Helvetica-Bold').text(formatCurrency(v.amount), sumRight, y);
        doc.font('Helvetica');
        y += 16;
        ...
        doc.text('Computer-generated document. No signature required. | REMS', pageW / 2, pageH - 24, { align: 'center', width: pageW - 80 });
        doc.end();
    });
}
//# sourceMappingURL=audit-grade-pdf-report.js.map